...
from pkgwh.cache import cache_errors, flat_hash
import pkgwh.eclass_cache
from pkgwh.cache.template import reconstruct_eclasses
from pkgwh.cache.mappings import ProtectedDict
...
class database(flat_hash.database):
	...
	def _parse_data(self, data, cpv):
		...
		if "_eclasses_" not in d:
			if "INHERITED" in d:
				if self.ec is None:
					self.ec = pkgwh.eclass_cache.cache(self.location[:-15])
	...
	def _setitem(self, cpv, values):
		...
		fp = os.path.join(self.location,cpv[:s],
			".update.%i.%s" % (pkgwh.getpid(), cpv[s+1:]))
		try:
