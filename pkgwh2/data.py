...
lchown = pkgwh._unicode_func_wrapper(lchown)

def _target_eprefix():
	"""
	Calculate the target EPREFIX, which may be different from
	pkgwh.const.EPREFIX due to cross-prefix support. The result
	is equivalent to pkgwh.settings["EPREFIX"], but the calculation
	is done without the expense of instantiating pkgwh.settings.
	@rtype: str
	@return: the target EPREFIX
	"""
	eprefix = os.environ.get("EPREFIX", pkgwh.const.EPREFIX)
	if eprefix:
		eprefix = pkgwh.util.normalize_path(eprefix)
	return eprefix

def _target_root():
	"""
	Calculate the target ROOT. The result is equivalent to
	pkgwh.settings["ROOT"], but the calculation
	is done without the expense of instantiating pkgwh.settings.
	@rtype: str
	...
	if not root:
		...
		root = os.sep
	root = pkgwh.util.normalize_path(root)
...
def _get_global(k):
	...
	if k == 'secpass':
		...
		if hasattr(portage, 'settings'):
			unprivileged = "unprivileged" in pkgwh.settings.features
		else:
		...
		if secpass >= 2:
			...
			encoding = pkgwh._encodings['content']
			cmd = [pkgwh._unicode_encode(x,
			...
			if os.WIFEXITED(status) and os.WEXITSTATUS(status) == os.EX_OK:
				for x in pkgwh._unicode_decode(myoutput,
	...
	# Avoid instantiating pkgwh.settings when the desired
...
class _GlobalProxy(pkgwh.proxy.objectproxy.ObjectProxy):

	__slots__ = ('_name',)

	def __init__(self, name):
		pkgwh.proxy.objectproxy.ObjectProxy.__init__(self)
