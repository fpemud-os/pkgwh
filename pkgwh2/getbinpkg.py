...
def make_metadata_dict(data):
	...
	warnings.warn("pkgwh.getbinpkg.make_metadata_dict() is deprecated",
		DeprecationWarning, stacklevel=2)

	myid, _myglob = data

	mydict = {}
	for k_bytes in pkgwh.xpak.getindex_mem(myid):
		k = _unicode_decode(k_bytes,
			encoding=_encodings['repo.content'], errors='replace')
		if k not in _all_metadata_keys and k != "CATEGORY":
			continue
		v = _unicode_decode(pkgwh.xpak.getitem(data, k_bytes),
			encoding=_encodings['repo.content'], errors='replace')
		mydict[k] = v
...
class ParseLinks(html_parser_HTMLParser):
	...
	def __init__(self):
		...
		warnings.warn("pkgwh.getbinpkg.ParseLinks is deprecated",
			DeprecationWarning, stacklevel=2)
...
def create_conn(baseurl, conn=None):
	...
	warnings.warn("pkgwh.getbinpkg.create_conn() is deprecated",
		DeprecationWarning, stacklevel=2)
...
def make_ftp_request(conn, address, rest=None, dest=None):
	...
	warnings.warn("pkgwh.getbinpkg.make_ftp_request() is deprecated",
		DeprecationWarning, stacklevel=2)
...
def make_http_request(conn, address, _params={}, headers={}, dest=None):
	...
	warnings.warn("pkgwh.getbinpkg.make_http_request() is deprecated",
		DeprecationWarning, stacklevel=2)
...
def match_in_array(array, prefix="", suffix="", match_both=1, allow_overlap=0):
	...
	warnings.warn("pkgwh.getbinpkg.match_in_array() is deprecated",
		DeprecationWarning, stacklevel=2)
...
def dir_get_list(baseurl, conn=None):
	...
	warnings.warn("pkgwh.getbinpkg.dir_get_list() is deprecated",
		DeprecationWarning, stacklevel=2)
...
def file_get_metadata(baseurl, conn=None, chunk_size=3000):
	...
	warnings.warn("pkgwh.getbinpkg.file_get_metadata() is deprecated",
		DeprecationWarning, stacklevel=2)
	...
	if data:
		xpaksize = pkgwh.xpak.decodeint(data[-8:-4])
		...
		myid = pkgwh.xpak.xsplit_mem(xpak_data)
		if not myid:
			myid = None, None
		del xpak_data
	else:
...
def file_get(baseurl=None, dest=None, conn=None, fcmd=None, filename=None,
	...
	if not fcmd:
		...
		warnings.warn("Use of pkgwh.getbinpkg.file_get() without the fcmd "
	...
	if "DISTDIR" not in variables:
		if dest is None:
			raise pkgwh.exception.MissingParameter(
	...
	from pkgwh.util import varexpand
	from pkgwh.process import spawn
	myfetch = pkgwh.util.shlex_split(fcmd)
...
def file_get_lib(baseurl, dest, conn=None):
	...
	warnings.warn("pkgwh.getbinpkg.file_get_lib() is deprecated",
		DeprecationWarning, stacklevel=2)
...
def dir_get_metadata(baseurl, conn=None, chunk_size=3000, verbose=1, usingcache=1, makepickle=None):
	...
	warnings.warn("pkgwh.getbinpkg.dir_get_metadata() is deprecated",
		DeprecationWarning, stacklevel=2)
...
class PackageIndex:
	...
	def write(self, pkgfile):
		...
		for metadata in sorted(self.packages,
			key=pkgwh.util.cmp_sort_key(_cmp_cpv)):
