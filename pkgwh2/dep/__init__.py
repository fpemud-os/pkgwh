...
pkgwh.proxy.lazyimport.lazyimport(globals(),
	'pkgwh.util:cmp_sort_key,writemsg',
)

from pkgwh import _encodings, _unicode_decode, _unicode_encode
from pkgwh.eapi import _get_eapi_attrs
from pkgwh.exception import InvalidAtom, InvalidData, InvalidDependString
from pkgwh.localization import _
from pkgwh.versions import _cp, _cpv, _pkg_str, _slot, _unknown_repo, _vr, \
	catpkgsplit, vercmp, ververify
import pkgwh.cache.mappings
...
def cpvequal(cpv1, cpv2):
	"""
	Example Usage:
		>>> from pkgwh.dep import cpvequal
	...
	try:
		...
		raise pkgwh.exception.PortageException(_("Invalid data '%s, %s', parameter was not a CPV") % (cpv1, cpv2))
...
def strip_empty(myarr):
	...
	warnings.warn(_("%s is deprecated and will be removed without replacement.") % \
		('pkgwh.dep.strip_empty',), DeprecationWarning, stacklevel=2)
...
def paren_reduce(mystr, _deprecation_warn=True):
	...
	"""
	if pkgwh._internal_caller and _deprecation_warn:
		warnings.warn(_("%s is deprecated and will be removed without replacement.") % \
			('pkgwh.dep.paren_reduce',), DeprecationWarning, stacklevel=2)
	mysplit = mystr.split()
	level = 0
	stack = [[]]
	need_bracket = False
...
class paren_normalize(list):
	...
	def __init__(self, src):
		if pkgwh._internal_caller:
			warnings.warn(_("%s is deprecated and will be removed without replacement.") % \
				('pkgwh.dep.paren_normalize',), DeprecationWarning, stacklevel=2)
		list.__init__(self)
...
def _use_reduce_cached(depstr, uselist, masklist, matchall, excludeall, \
	...
	if opconvert and flat:
		raise ValueError("pkgwh.dep.use_reduce: 'opconvert' and 'flat' are mutually exclusive")

	if matchall and matchnone:
		raise ValueError("pkgwh.dep.use_reduce: 'matchall' and 'matchnone' are mutually exclusive")
	...
	if isinstance(depstr, list):
		if pkgwh._internal_caller:
			warnings.warn(_("Passing paren_reduced dep arrays to %s is deprecated. " + \
				...
				('pkgwh.dep.use_reduce',), DeprecationWarning, stacklevel=2)
...
def dep_opconvert(deplist):
	...
	if pkgwh._internal_caller:
		warnings.warn(_("%s is deprecated. Use %s with the opconvert parameter set to True instead.") % \
			('pkgwh.dep.dep_opconvert', 'pkgwh.dep.use_reduce'), DeprecationWarning, stacklevel=2)
...
def flatten(mylist):
	...
	if pkgwh._internal_caller:
		warnings.warn(_("%s is deprecated and will be removed without replacement.") % \
			('pkgwh.dep.flatten',), DeprecationWarning, stacklevel=2)
...
class _use_dep:
	...
	def __repr__(self):
		return "pkgwh.dep._use_dep(%s)" % repr(self.tokens)
...
class ExtendedAtomDict(pkgwh.cache.mappings.MutableMapping):
	"""
