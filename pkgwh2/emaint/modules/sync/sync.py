...
pkgwh._sync_mode = True
from pkgwh.output import bold, red, create_color_func
from pkgwh._global_updates import _global_updates
from pkgwh.sync.controller import SyncManager
from pkgwh.util.digraph import digraph
from pkgwh.util.futures import asyncio
from pkgwh.util._async.AsyncScheduler import AsyncScheduler
...
class SyncRepos:
	...
	def __init__(self, emerge_config=None, emerge_logging=False):
		...
		if emerge_config is None:
			...
			# --package-moves=n.
			cmdline = pkgwh.util.shlex_split(
			...
			if hasattr(portage, 'settings'):
				...
				pkgwh._reset_legacy_globals()
				# update redundant global variables, for consistency
				# and in order to conserve memory
				pkgwh.settings = emerge_config.target_config.settings
				pkgwh.db = emerge_config.trees
	...
	def _check_updates(self):
		mybestpv = self.emerge_config.target_config.trees['porttree'].dbapi.xmatch(
			"bestmatch-visible", pkgwh.const.PORTAGE_PACKAGE_ATOM)
		mypvs = pkgwh.best(
