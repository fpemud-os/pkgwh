...
class MoveHandler:
	...
	def _grab_global_updates(self):
		from pkgwh.update import grab_updates, parse_updates
		retupdates = {}
		errors = []
	...
	def check(self, **kwargs):
		...
		for i, cpv in enumerate(cpv_all):
			...
			if not updates:
				continue
			metadata_updates = \
				pkgwh.update_dbentries(updates, metadata, parent=pkg)
			if metadata_updates:
...
class MoveInstalled(MoveHandler):
	...
	def __init__(self):
		...
		MoveHandler.__init__(self, pkgwh.db[eroot]["vartree"], pkgwh.db[eroot]["porttree"])
...
class MoveBinary(MoveHandler):
	...
	def __init__(self):
		eroot = pkgwh.settings['EROOT']
		MoveHandler.__init__(self, pkgwh.db[eroot]["bintree"], pkgwh.db[eroot]['porttree'])
