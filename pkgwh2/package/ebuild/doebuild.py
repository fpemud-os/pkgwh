...
pkgwh.proxy.lazyimport.lazyimport(globals(),
	'pkgwh.package.ebuild.config:check_config_instance',
	'pkgwh.package.ebuild.digestcheck:digestcheck',
	'pkgwh.package.ebuild.digestgen:digestgen',
	'pkgwh.package.ebuild.fetch:_drop_privs_userfetch,_want_userfetch,fetch',
	'pkgwh.package.ebuild.prepare_build_dirs:_prepare_fake_distdir',
	'pkgwh.package.ebuild._ipc.QueryCommand:QueryCommand',
	'pkgwh.dep._slot_operator:evaluate_slot_operator_equal_deps',
	'pkgwh.package.ebuild._spawn_nofetch:spawn_nofetch',
	'pkgwh.util.elf.header:ELFHeader',
	'pkgwh.dep.soname.multilib_category:compute_multilib_category',
	'pkgwh.util._desktop_entry:validate_desktop_entry',
	'pkgwh.util._dyn_libs.NeededEntry:NeededEntry',
	'pkgwh.util._dyn_libs.soname_deps:SonameDepsProcessor',
	'pkgwh.util._async.SchedulerInterface:SchedulerInterface',
	'pkgwh.util._eventloop.global_event_loop:global_event_loop',
	'pkgwh.util.ExtractKernelVersion:ExtractKernelVersion'
)

from pkgwh import bsd_chflags, \
	eapi_is_supported, merge, os, selinux, shutil, \
	unmerge, _encodings, _os_merge, \
	_shell_quote, _unicode_decode, _unicode_encode
from pkgwh.const import EBUILD_SH_ENV_FILE, EBUILD_SH_ENV_DIR, \
	EBUILD_SH_BINARY, INVALID_ENV_FILE, MISC_SH_BINARY, PORTAGE_PYM_PACKAGES
from pkgwh.data import pkgwh_gid, portage_uid, secpass, \
	uid, userpriv_groups
from pkgwh.dbapi.porttree import _parse_uri_map
from pkgwh.dep import Atom, check_required_use, \
	human_readable_required_use, paren_enclose, use_reduce
from pkgwh.eapi import (eapi_exports_KV, eapi_exports_merge_type,
...
from pkgwh.elog import elog_process, _preload_elog_modules
from pkgwh.elog.messages import eerror, eqawarn
from pkgwh.exception import (DigestException, FileNotFound,
...
from pkgwh.output import colormap
from pkgwh.package.ebuild.prepare_build_dirs import prepare_build_dirs
from pkgwh.process import find_binary
from pkgwh.util import ( apply_recursive_permissions,
...
from pkgwh.util.cpuinfo import get_cpu_count
from pkgwh.util.lafilefixer import rewrite_lafile
from pkgwh.util.compression_probe import _compressors
from pkgwh.util.futures import asyncio
from pkgwh.util.futures.executor.fork import ForkExecutor
from pkgwh.util.path import first_existing
from pkgwh.util.socks5 import get_socks5_proxy
from pkgwh.versions import _pkgsplit
...
def _doebuild_path(settings, eapi=None):
	...
	if portage_bin_path[0] != pkgwh.const.PORTAGE_BIN_PATH:
		# Add a fallback path for restarting failed builds (bug 547086)
		portage_bin_path.append(pkgwh.const.PORTAGE_BIN_PATH)
	...
	prefixes = []
	# settings["EPREFIX"] should take priority over pkgwh.const.EPREFIX
	if pkgwh.const.EPREFIX != settings["EPREFIX"] and settings["ROOT"] == os.sep:
		prefixes.append(settings["EPREFIX"])
	prefixes.append(pkgwh.const.EPREFIX)
...
def doebuild_environment(myebuild, mydo, myroot=None, settings=None,
	...
	mysettings['PORTAGE_PYTHON'] = pkgwh._python_interpreter
	...
	if "COLUMNS" not in mysettings:
		...
		if columns is None:
			rows, columns = pkgwh.output.get_term_size()
			if columns < 1:
...
def doebuild(myebuild, mydo, _unused=DeprecationWarning, settings=None, debug=0, listonly=0,
	...
	@type settings: instance of pkgwh.config
	...
	if _unused is not DeprecationWarning:
		warnings.warn("The third parameter of the "
			"pkgwh.doebuild() is deprecated. Instead "
			"settings['EROOT'] is used.",
			DeprecationWarning, stacklevel=2)

	if dbkey is not DeprecationWarning:
		warnings.warn("pkgwh.doebuild() called "
	...
	if mydbapi is None:
		mydbapi = pkgwh.db[myroot][tree].dbapi

	if vartree is None and mydo in ("merge", "qmerge", "unmerge"):
		vartree = pkgwh.db[myroot]["vartree"]
	...
	if "strict" in features and \
		...
		not pkgwh._doebuild_manifest_exempt_depend and \
	...
	try:
		if mydo in ("digest", "manifest", "help"):
			...
			pkgwh._doebuild_manifest_exempt_depend += 1
		...
		if eapi_exports_replace_vars(mysettings["EAPI"]) and \
			...
			if not vartree:
				...
				cpv_slot = "%s%s%s" % \
					(cpv.cp, pkgwh.dep._slot_separator, cpv.slot)
				mysettings["REPLACING_VERSIONS"] = " ".join(
					set(pkgwh.versions.cpv_getversion(match) \
		...
		if regular_actionmap_phase:
			...
			if mydo == "package":
				...
				if hasattr(portage, 'db'):
					bintree = pkgwh.db[mysettings['EROOT']]['bintree']
					...
					parent_dir = os.path.join(mysettings["PKGDIR"],
						mysettings["CATEGORY"])
					pkgwh.util.ensure_dirs(parent_dir)
...
def _spawn_actionmap(settings):
	...
	if not pkgwh.process.sandbox_capable:
		nosandbox = True
...
def _validate_deps(mysettings, myroot, mydo, mydbapi):
	...
	if msgs:
		pkgwh.util.writemsg_level(_("Error(s) in metadata for '%s':\n") % \
			(mysettings.mycpv,), level=logging.ERROR, noiselevel=-1)
		for x in msgs:
			pkgwh.util.writemsg_level(x,
...
def spawn(mystring, mysettings, debug=False, free=False, droppriv=False,
	...
	use the pkgwh.process spawn* family of functions.

	This function waits for the process to terminate.

	@param mystring: Command to run
	@type mystring: String
	@param mysettings: Either a Dict of Key,Value pairs or an instance of pkgwh.config
	...
	if "PORTAGE_BUILD_USER" not in mysettings:
		...
		try:
			...
			if portage_build_uid == 0:
				...
				user = pkgwh.data._portage_username
		if user is not None:
	...
	if "PORTAGE_BUILD_GROUP" not in mysettings:
		...
		try:
			...
			if portage_build_gid == 0:
				...
				group = pkgwh.data._portage_grpname
	...
	if not free and not (fakeroot or pkgwh.process.sandbox_capable):
		free = True
	...
	if free or "SANDBOX_ACTIVE" in os.environ:
		...
		spawn_func = pkgwh.process.spawn_bash
	elif fakeroot:
		keywords["opt_name"] += " fakeroot"
		keywords["fakeroot_state"] = os.path.join(mysettings["T"], "fakeroot.state")
		spawn_func = pkgwh.process.spawn_fakeroot
	else:
		keywords["opt_name"] += " sandbox"
		spawn_func = pkgwh.process.spawn_sandbox
...
def spawnebuild(mydo, actionmap, mysettings, debug, alwaysdep=0,
	...
	if returnpid:
		warnings.warn("pkgwh.spawnebuild() called "
			"with returnpid parameter enabled. This usage will "
			"not be supported in the future.",
			DeprecationWarning, stacklevel=2)
...
def _post_src_install_uid_fix(mysettings, out):
	...
	desktop_file_validate = \
		pkgwh.process.find_binary("desktop-file-validate") is not None
		...
		for parent, dirs, files in os.walk(destdir):
			...
			for fname in chain(dirs, files):
				...
				if fixlafiles and \
					...
					except pkgwh.exception.InvalidData as e:
						needs_update = False
...
def _post_src_install_soname_symlinks(mysettings, out):
	...
	if qa_prebuilt:
		qa_prebuilt = re.compile("|".join(
			...
			for x in pkgwh.util.shlex_split(qa_prebuilt)))
	...
	libpaths = set(pkgwh.util.getlibpaths(
	...
	needed_file = pkgwh.util.atomic_ofstream(needed_filename,
...
def _prepare_self_update(settings):
	...
	if pkgwh._bin_path != pkgwh.const.PORTAGE_BIN_PATH:
		return
	...
	pkgwh.util.ensure_dirs(build_prefix)
	base_path_tmp = tempfile.mkdtemp(
		"", "._portage_reinstall_.", build_prefix)
	pkgwh.process.atexit_register(shutil.rmtree, base_path_tmp)

	orig_bin_path = pkgwh._bin_path
	pkgwh._bin_path = os.path.join(base_path_tmp, "bin")
	shutil.copytree(orig_bin_path, pkgwh._bin_path, symlinks=True)

	orig_pym_path = pkgwh._pym_path
	pkgwh._pym_path = os.path.join(base_path_tmp, "lib")
	os.mkdir(pkgwh._pym_path)
	for pmod in PORTAGE_PYM_PACKAGES:
		shutil.copytree(os.path.join(orig_pym_path, pmod),
			os.path.join(pkgwh._pym_path, pmod),
			symlinks=True)

	for dir_path in (base_path_tmp, pkgwh._bin_path, pkgwh._pym_path):
		os.chmod(dir_path, 0o755)

def _handle_self_update(settings, vardb):
	cpv = settings.mycpv
	if settings["ROOT"] == "/" and \
		pkgwh.dep.match_from_list(
		pkgwh.const.PORTAGE_PACKAGE_ATOM, [cpv]):
