...
import pkgwh
from pkgwh import os, eapi_is_supported, _encodings, _unicode_encode
from pkgwh.const import CUSTOM_PROFILE_PATH, GLOBAL_CONFIG_PATH, \
	PROFILE_PATH, USER_CONFIG_PATH
from pkgwh.eapi import eapi_allows_directories_on_profile_level_and_repository_level
from pkgwh.exception import DirectoryNotFound, InvalidLocation, ParseError
from pkgwh.localization import _
from pkgwh.util import ensure_dirs, grabfile, \
...
from pkgwh.repository.config import parse_layout_conf, \
...
class LocationsManager:
	...
	def __init__(self, config_root=None, eprefix=None, config_profile_path=None, local_config=True, \
		...
		if self.eprefix is None:
			self.eprefix = pkgwh.const.EPREFIX
		elif self.eprefix:
		...
		if self.config_root is None:
			self.config_root = pkgwh.const.EPREFIX + os.sep
		...
		self.broot = pkgwh.const.EPREFIX
	...
	def set_root_override(self, root_overwrite=None):
		...
		if pkgwh.const.EPREFIX:
			self.global_config_path = os.path.join(pkgwh.const.EPREFIX,
