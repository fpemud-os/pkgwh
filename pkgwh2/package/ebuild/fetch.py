...
pkgwh.proxy.lazyimport.lazyimport(globals(),
	'pkgwh.package.ebuild.config:check_config_instance,config',
	'pkgwh.package.ebuild.doebuild:doebuild_environment,' + \
		'_doebuild_spawn',
	'pkgwh.package.ebuild.prepare_build_dirs:prepare_build_dirs',
	'pkgwh.util:atomic_ofstream',
	'pkgwh.util.configparser:SafeConfigParser,read_configs,' +
		'ConfigParserError',
	'pkgwh.util.install_mask:_raise_exc',
	'pkgwh.util._urlopen:urlopen',
)

from pkgwh import os, selinux, shutil, _encodings, \
	_movefile, _shell_quote, _unicode_encode
from pkgwh.checksum import (get_valid_checksum_keys, perform_md5, verify_all,
	_filter_unaccelarated_hashes, _hash_filter, _apply_hash_filter,
	checksum_str)
from pkgwh.const import BASH_BINARY, CUSTOM_MIRRORS_FILE, \
	GLOBAL_CONFIG_PATH
from pkgwh.data import pkgwh_gid, portage_uid, userpriv_groups
from pkgwh.exception import FileNotFound, OperationNotPermitted, \
	PortageException, TryAgain
from pkgwh.localization import _
from pkgwh.locks import lockfile, unlockfile
from pkgwh.output import colorize, EOutput
...
from pkgwh.process import spawn
...
def _want_userfetch(settings):
	...
	@type settings: pkgwh.package.ebuild.config.config
	@return: True if desirable, False otherwise
	"""
	return ('userfetch' in settings.features and
		pkgwh.data.secpass >= 2 and os.getuid() == 0)


def _drop_privs_userfetch(settings):
	"""
	Drop privileges for userfetch, and update pkgwh.data.secpass
...
def _ensure_distdir(settings, distdir):
	...
	@type distdir: str
	@raise PortageException: pkgwh.exception wrapper exception
	"""
	...
	userfetch = pkgwh.data.secpass >= 2 and "userfetch" in settings.features
	userpriv = pkgwh.data.secpass >= 2 and "userpriv" in settings.features
	...
	def get_filenames(self, distdir):
		for dirpath, dirnames, filenames in os.walk(distdir,
			...
			for filename in filenames:
				try:
					yield pkgwh._unicode_decode(filename, errors='strict')
...
class FilenameHashLayout:
	...
	def get_filenames(self, distdir):
		...
		for c in self.cutoffs:
			...
			pattern += c * '[0-9a-f]' + '/'
		pattern += '*'
		for x in glob.iglob(pkgwh._unicode_encode(os.path.join(distdir, pattern), errors='strict')):
			try:
				yield pkgwh._unicode_decode(x, errors='strict').rsplit('/', 1)[1]
			except UnicodeDecodeError:
				# Ignore it. Distfiles names must have valid UTF8 encoding.
				pass
...
def fetch(myuris, mysettings, listonly=0, fetchonly=0,
	...
	userfetch = pkgwh.data.secpass >= 2 and "userfetch" in features
