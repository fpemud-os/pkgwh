...
from pkgwh import normalize_path
...
class tbz2:
	...
	def recompose_mem(self, xpdata, break_hardlinks=True):
		...
		if break_hardlinks and self.filestat and self.filestat.st_nlink > 1:
			tmp_fname = "%s.%d" % (self.file, pkgwh.getpid())
			copyfile(self.file, tmp_fname)
			try:
				pkgwh.util.apply_stat_permissions(self.file, self.filestat)
			except pkgwh.exception.OperationNotPermitted:
				pass
