...
import pkgwh
from pkgwh import os, _encodings, _unicode_decode
from pkgwh.exception import (DirectoryNotFound, FileNotFound,
...
def _lockfile_iteration(mypath, wantnewlockfile=False, unlinkfile=False,
	...
	portage_gid = int(pkgwh.data.portage_gid)
	...
	if isinstance(mypath, str):
		...
		try:
			...
			if not preexisting:
				try:
					if pkgwh.data.secpass >= 1 and os.stat(lockfilename).st_gid != portage_gid:
	...
	locking_method = pkgwh._eintr_func_wrapper(_get_lock_fn())
	try:
...
def hardlock_name(path):
	...
	return os.path.join(base, ".%s.hardlock-%s-%s" %
		(tail, pkgwh._decode_argv([os.uname()[1]])[0], pkgwh.getpid()))
...
def hardlink_lockfile(lockfilename, max_wait=DeprecationWarning,
	...
	if max_wait is not DeprecationWarning:
		warnings.warn("The 'max_wait' parameter of "
			"pkgwh.locks.hardlink_lockfile() is now unused. Use "
		...
		try:
			...
			try:
				...
				if not preexisting:
					...
					if pkgwh.data.secpass >= 1 and myfd_st.st_gid != portage_gid:
...
def hardlock_cleanup(path, remove_all_locks=False):
	myhost = pkgwh._decode_argv([os.uname()[1]])[0]
