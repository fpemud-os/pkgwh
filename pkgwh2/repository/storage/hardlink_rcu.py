...
from pkgwh.repository.storage.interface import (
	RepoStorageException,
	RepoStorageInterface,
)
from pkgwh.util.futures import asyncio
from pkgwh.util.futures.compat_coroutine import (
	coroutine,
	coroutine_return,
)
...
class HardlinkRcuRepoStorage(RepoStorageInterface):
	...
	def init_update(self, loop=None):
		...
		pkgwh.util.ensure_dirs(self._storage_location)

		if self._latest_canonical is not None:
			pkgwh.util.ensure_dirs(update_location)
			pkgwh.util.apply_stat_permissions(update_location,
	...
	def commit_update(self, loop=None):
		...
		try:
			...
			snapshots = []
			pkgwh.util.ensure_dirs(self._snapshots_dir)
			pkgwh.util.apply_stat_permissions(self._snapshots_dir,
