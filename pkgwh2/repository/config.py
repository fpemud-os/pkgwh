...
import pkgwh
from pkgwh import eclass_cache, os
from pkgwh.checksum import get_valid_checksum_keys
from pkgwh.const import (PORTAGE_BASE_PATH, REPO_NAME_LOC, USER_CONFIG_PATH)
from pkgwh.eapi import eapi_allows_directories_on_profile_level_and_repository_level
from pkgwh.env.loaders import KeyValuePairFileLoader
from pkgwh.util import (normalize_path, read_corresponding_eapi_file, shlex_split,
	stack_lists, writemsg, writemsg_level, _recursive_file_list)
from pkgwh.util.configparser import (SafeConfigParser, ConfigParserError,
	read_configs)
from pkgwh.util._path import isdir_raise_eaccess
...
class RepoConfig:
	...
	def __init__(self, name, repo_opts, local_config=True):
		...
		if location is not None and location.strip():
			if os.path.isdir(location) or pkgwh._sync_mode:
	...
	def iter_pregenerated_caches(self, auxdbkeys, readonly=True, force=False):
		...
		for fmt in formats:
			name = None
			if fmt == 'pms':
				from pkgwh.cache.metadata import database
				name = 'metadata/cache'
			elif fmt == 'md5-dict':
				from pkgwh.cache.flat_hash import md5_database as database
	...
	def __repr__(self):
		return "<pkgwh.repository.config.RepoConfig(name=%r, location=%r)>" % (self.name, _unicode_decode(self.location))
	...
	def __init__(self, paths, settings):
		...
		if main_repo is None or main_repo not in prepos:
			...
			if main_repo is not None:
				...
				if portdir and not pkgwh._sync_mode:
		...
		for repo_name, repo in prepos.items():
			...
			if repo._masters_orig is None and self.mainRepo() and \
				repo.name != self.mainRepo().name and not pkgwh._sync_mode:
		...
		for (name, r) in self.prepos.items():
			if name != 'DEFAULT':
				if r.location is None:
					...
					if not isdir_raise_eaccess(r.location) and not pkgwh._sync_mode:
	...
	if raw_formats is None:
		...
		if unknown:
			...
			warnings.warn((_("Repository named '%(repo_name)s' has unsupported "
				...
				"'%(layout_filename)s; please upgrade pkgwh.") %
	...
	try:
		...
		if 'profile-default-eapi' not in raw_formats:
			warnings.warn((_("Repository named '%(repo_name)s' has "
				...
				layout_filename=layout_filename)),
				SyntaxWarning)
		elif not pkgwh.eapi_is_supported(eapi):
			warnings.warn((_("Repository named '%(repo_name)s' has "
