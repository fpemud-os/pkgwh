...
import pkgwh
from pkgwh import _encodings, os, shutil
from pkgwh.env.loaders import KeyValuePairFileLoader
from pkgwh.localization import _
from pkgwh.util import shlex_split, varexpand
from pkgwh.util.path import iter_parents
...
def diffstatusoutput(cmd, file1, file2):
	...
	args = [pkgwh._unicode_encode(x, errors='strict') for x in args]
	proc = subprocess.Popen(args,
		stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
	output = pkgwh._unicode_decode(proc.communicate()[0])
...
def read_config(mandatory_opts):
	...
	if pkgwh._not_installed:
		config_path = os.path.join(pkgwh.PORTAGE_BASE_PATH, "cnf", "dispatch-conf.conf")
	else:
