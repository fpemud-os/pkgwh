...
from pkgwh.util import writemsg_level, shlex_split
from pkgwh.util.futures import asyncio
from pkgwh.output import create_color_func, EOutput
...
class GitSync(NewBase):
	...
	def __init__(self):
		NewBase.__init__(self, "git", pkgwh.const.GIT_PACKAGE_ATOM)
	...
	def new(self, **kwargs):
		...
		git_cmd = "%s clone%s %s ." % (self.bin_command, git_cmd_opts,
			pkgwh._shell_quote(sync_uri))
		writemsg_level(git_cmd + "\n")

		exitcode = pkgwh.process.spawn_bash("cd %s ; exec %s" % (
				pkgwh._shell_quote(self.repo.location), git_cmd),
			**self.spawn_kwargs)
	...
	def update(self):
		...
		try:
			remote_branch = pkgwh._unicode_decode(
				subprocess.check_output([self.bin_command, 'rev-parse',
				'--abbrev-ref', '--symbolic-full-name', '@{upstream}'],
				cwd=pkgwh._unicode_encode(self.repo.location))).rstrip('\n')
		...
		if shallow:
			...
			if quiet:
				gc_cmd.append('--quiet')
			exitcode = pkgwh.process.spawn(gc_cmd,
				cwd=pkgwh._unicode_encode(self.repo.location),
				**self.spawn_kwargs)
			if exitcode != os.EX_OK:
		...
		exitcode = pkgwh.process.spawn(merge_cmd,
			cwd=pkgwh._unicode_encode(self.repo.location),
			**self.spawn_kwargs)
	...
	def retrieve_head(self, **kwargs):
		...
		try:
			ret = (os.EX_OK,
				pkgwh._unicode_decode(subprocess.check_output(rev_cmd,
				cwd=pkgwh._unicode_encode(self.repo.location))))
		except subprocess.CalledProcessError:
			ret = (1, False)
		return ret
