...
class SyncManager:
	...
	def __getattr__(self, name):
		if name == 'async':
			warnings.warn("pkgwh.sync.controller.SyncManager.async "
	...
	def perform_post_sync_hook(self, reponame, dosyncuri='', repolocation=''):
		...
		for filepath in _hooks:
			...
			if reponame:
				retval = pkgwh.process.spawn(
					[filepath, reponame, dosyncuri, repolocation],
					env=self.settings.environ())
			else:
				retval = pkgwh.process.spawn([filepath],
					env=self.settings.environ())
			if retval != os.EX_OK:
