...
def chk_updated_info_files(root, infodirs, prev_mtimes):
	...
	if os.path.exists("/usr/bin/install-info"):
		...
		for z in infodirs:
			if z == '':
				continue
			inforoot = pkgwh.util.normalize_path(root + z)
		...
		if not regen_infodirs:
			pkgwh.util.writemsg_stdout("\n")
			if pkgwh.util.noiselimit >= 0:
			...
			if badcount:
				out.eerror("Processed %d info files; %d errors." % \
					(icount, badcount))
				pkgwh.util.writemsg_level(errmsg,
					level=logging.ERROR, noiselevel=-1)
			else:
				if icount > 0 and pkgwh.util.noiselimit >= 0:
