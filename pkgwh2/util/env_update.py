...
import pkgwh
from pkgwh import os, _encodings, _unicode_decode, _unicode_encode
from pkgwh.checksum import prelink_capable
from pkgwh.data import ostype
from pkgwh.exception import ParseError
from pkgwh.localization import _
from pkgwh.process import find_binary
from pkgwh.util import atomic_ofstream, ensure_dirs, getconfig, \
	normalize_path, writemsg
from pkgwh.util.listdir import listdir
from pkgwh.dbapi.vartree import vartree
...
def env_update(makelinks=1, target_root=None, prev_mtimes=None, contents=None,
	...
	@param target_root: root that is passed to the ldconfig -r option,
		defaults to pkgwh.settings["ROOT"].
	...
	if vardbapi is None:
		if isinstance(env, config):
			...
			if target_root is None:
				eprefix = pkgwh.settings["EPREFIX"]
				target_root = pkgwh.settings["ROOT"]
				target_eroot = pkgwh.settings['EROOT']
			else:
				eprefix = pkgwh.const.EPREFIX
				target_eroot = os.path.join(target_root,
					eprefix.lstrip(os.sep))
				target_eroot = target_eroot.rstrip(os.sep) + os.sep
			if hasattr(portage, "db") and target_eroot in pkgwh.db:
				vardbapi = pkgwh.db[target_eroot]["vartree"].dbapi
			else:
...
def _env_update(makelinks, target_root, prev_mtimes, contents, env,
	...
	if prev_mtimes is None:
		prev_mtimes = pkgwh.mtimedb["ldpath"]
	if env is None:
	...
	if prelink_capable:
		...
		newprelink = atomic_ofstream(os.path.join(prelink_d, 'pkgwh.conf'))
