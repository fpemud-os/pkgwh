...
from pkgwh import load_mod
from pkgwh import _unicode_decode
from pkgwh import _unicode_encode
from pkgwh import _encodings
from pkgwh.const import USER_CONFIG_PATH, GLOBAL_CONFIG_PATH
from pkgwh.const import VCS_DIRS
from pkgwh.const import _ENABLE_SET_CONFIG
from pkgwh.exception import PackageSetNotFound
from pkgwh.localization import _
from pkgwh.util import writemsg_level
from pkgwh.util.configparser import (SafeConfigParser,
...
class SetConfig:
	...
	def _create_default_config(self):
		...
		minimize the diff between branches.
			...
			[world]
			class = pkgwh.sets.base.DummyPackageSet
			packages = @selected @system

			[selected]
			class = pkgwh.sets.files.WorldSelectedSet

			[system]
			class = pkgwh.sets.profiles.PackagesSystemSet
		...
		parser.set("world", "class", "pkgwh.sets.base.DummyPackageSet")
		parser.set("world", "packages", "@profile @selected @system")

		parser.remove_section("profile")
		parser.add_section("profile")
		parser.set("profile", "class", "pkgwh.sets.ProfilePackageSet.ProfilePackageSet")

		parser.remove_section("selected")
		parser.add_section("selected")
		parser.set("selected", "class", "pkgwh.sets.files.WorldSelectedSet")

		parser.remove_section("selected-packages")
		parser.add_section("selected-packages")
		parser.set("selected-packages", "class", "pkgwh.sets.files.WorldSelectedPackagesSet")

		parser.remove_section("selected-sets")
		parser.add_section("selected-sets")
		parser.set("selected-sets", "class", "pkgwh.sets.files.WorldSelectedSetsSet")

		parser.remove_section("system")
		parser.add_section("system")
		parser.set("system", "class", "pkgwh.sets.profiles.PackagesSystemSet")

		parser.remove_section("security")
		parser.add_section("security")
		parser.set("security", "class", "pkgwh.sets.security.NewAffectedSet")

		parser.remove_section("usersets")
		parser.add_section("usersets")
		parser.set("usersets", "class", "pkgwh.sets.files.StaticFileSet")
		...
		parser.set("live-rebuild", "class", "pkgwh.sets.dbapi.VariableSet")
		...
		parser.set("deprecated-live-rebuild", "class", "pkgwh.sets.dbapi.VariableSet")
		parser.set("deprecated-live-rebuild", "variable", "INHERITED")
		parser.set("deprecated-live-rebuild", "includes", " ".join(sorted(pkgwh.const.LIVE_ECLASSES)))

		parser.remove_section("module-rebuild")
		parser.add_section("module-rebuild")
		parser.set("module-rebuild", "class", "pkgwh.sets.dbapi.OwnerSet")
		parser.set("module-rebuild", "files", "/lib/modules")

		parser.remove_section("preserved-rebuild")
		parser.add_section("preserved-rebuild")
		parser.set("preserved-rebuild", "class", "pkgwh.sets.libs.PreservedLibraryConsumerSet")

		parser.remove_section("x11-module-rebuild")
		parser.add_section("x11-module-rebuild")
		parser.set("x11-module-rebuild", "class", "pkgwh.sets.dbapi.OwnerSet")
	...
	def _parse(self, update=False):
		...
		for sname in parser.sections():
			...
			if not parser.has_option(sname, "class"):
				classname = "pkgwh._sets.files.StaticFileSet"
			else:
				classname = parser.get(sname, "class")

			if classname.startswith('pkgwh.sets.'):
			...
			try:
				...
				try:
					setclass = load_mod("pkgwh._sets." + classname)
				except (ImportError, AttributeError):
