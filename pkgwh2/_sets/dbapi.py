...
from pkgwh.exception import pkgwhKeyError
from pkgwh.versions import best, catsplit, vercmp
from pkgwh.dep import Atom, use_reduce
from pkgwh.dep._slot_operator import strip_slots
from pkgwh.localization import _
from pkgwh._sets.base import PackageSet
from pkgwh._sets import SetConfigError, get_boolean
import pkgwh
...
class OwnerSet(PackageSet):
	...
	def singleBuilder(cls, options, settings, trees):
		...
		if exclude_files is not None:
			exclude_files = frozenset(pkgwh.util.shlex_split(exclude_files))
		return cls(vardb=trees["vartree"].dbapi, exclude_files=exclude_files,
			files=frozenset(pkgwh.util.shlex_split(options["files"])))
