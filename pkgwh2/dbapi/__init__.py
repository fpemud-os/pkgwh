...
pkgwh.proxy.lazyimport.lazyimport(globals(),
	'pkgwh.dbapi.dep_expand:dep_expand@_dep_expand',
	'pkgwh.dep:Atom,match_from_list,_match_slot',
	'pkgwh.output:colorize',
	'pkgwh.util:cmp_sort_key,writemsg',
	'pkgwh.versions:catsplit,catpkgsplit,vercmp,_pkg_str',
)

from pkgwh.const import MERGING_IDENTIFIER

from pkgwh import os
from pkgwh import auxdbkeys
...
class dbapi:
	...
	def update_ents(self, updates, onProgress=None, onUpdate=None):
		...
		for i, cpv in enumerate(cpv_all):
			...
			metadata_updates = \
				pkgwh.update_dbentries(updates_list, metadata, parent=pkg)
			if metadata_updates:
