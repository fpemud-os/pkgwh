...
from pkgwh.const import PRIVATE_PATH
from pkgwh.dep import _slot_separator
from pkgwh.localization import _
from pkgwh.util import grabfile
from pkgwh.versions import pkgsplit, vercmp
...
def match(atom, dbapi, match_type="default"):
	"""
	wrapper that calls revisionMatch() or pkgwh.dbapi.dbapi.match() depending on
	the given atom.

	@type	atom: string
	@param	atom: a <~ or >~ atom or a normal portage atom that contains the atom to match against
	@type	dbapi: pkgwh.dbapi.dbapi
...
def getMinUpgrade(vulnerableList, unaffectedList, portdbapi, vardbapi, minimize=True):
	...
	@type	portdbapi:	pkgwh.dbapi.porttree.portdbapi
	@param	portdbapi:	Ebuild repository
	@type	vardbapi:	pkgwh.dbapi.vartree.vardbapi
