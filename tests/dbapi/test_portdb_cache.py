...
from pkgwh.const import (BASH_BINARY, PORTAGE_PYM_PATH, USER_CONFIG_PATH)
...
class PortdbCacheTestCase(TestCase):
	...
	def testPortdbCache(self):
		...
		portage_python = pkgwh._python_interpreter
		...
		test_commands = (
			...
			python_cmd + (textwrap.dedent("""
				...
				if pkgwh.portdb.repositories['test_repo'].location in pkgwh.portdb._pregen_auxdb:
					sys.exit(1)
			"""),),
			...
			python_cmd + (textwrap.dedent("""
				import os, sys, portage
				if pkgwh.portdb.repositories['test_repo'].location not in pkgwh.portdb._pregen_auxdb:
					sys.exit(1)
			"""),),
			python_cmd + (textwrap.dedent("""
				import os, sys, portage
				from pkgwh.cache.flat_hash import md5_database
				if not isinstance(pkgwh.portdb._pregen_auxdb[pkgwh.portdb.repositories['test_repo'].location], md5_database):
					sys.exit(1)
			"""),),

			(BASH_BINARY, "-c", "echo %s > %s" %
				tuple(map(pkgwh._shell_quote,
			...
			(portage_python, "-b", "-Wd", "-Wi::DeprecationWarning", "-c") + (textwrap.dedent("""
				import os, sys, portage
				from pkgwh.cache.metadata import database as pms_database
				if not isinstance(pkgwh.portdb._pregen_auxdb[pkgwh.portdb.repositories['test_repo'].location], pms_database):
					sys.exit(1)
			"""),),

			# Test auto-detection and preference for md5-cache when both
			# cache formats are available but layout.conf is absent.
			(BASH_BINARY, "-c", "rm %s" % pkgwh._shell_quote(layout_conf_path)),
			python_cmd + (textwrap.dedent("""
				import os, sys, portage
				if pkgwh.portdb.repositories['test_repo'].location not in pkgwh.portdb._pregen_auxdb:
					sys.exit(1)
