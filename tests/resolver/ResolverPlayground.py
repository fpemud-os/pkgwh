...
from pkgwh.const import (GLOBAL_CONFIG_PATH, USER_CONFIG_PATH)
from pkgwh.process import find_binary
from pkgwh.dep import Atom, _repo_separator
from pkgwh.package.ebuild.config import config
...
from pkgwh.tests import cnf_path
from pkgwh.util import ensure_dirs, normalize_path
from pkgwh.versions import catsplit
...
"""
	...
	def __init__(self, ebuilds={}, binpkgs={}, installed={}, profile={}, repo_configs={}, \
		...
		if eprefix is None:
			...
			try:
				...
				for x in essential_binaries:
					path = find_binary(x)
					if path is None:
						raise pkgwh.exception.CommandNotFound(x)
		...
		# Tests may override pkgwh.const.EPREFIX in order to
		# simulate a prefix installation. It's reasonable to do
		# this because tests should be self-contained such that
		# the "real" value of pkgwh.const.EPREFIX is entirely
		# irrelevant (see bug #492932).
		pkgwh.const.EPREFIX = self.eprefix.rstrip(os.sep)
	...
	def _create_binpkgs(self, binpkgs):
		...
		for cpv, metadata in items:
			...
			t = pkgwh.xpak.tbz2(binpkg_path)
			t.recompose_mem(pkgwh.xpak.xpak_mem(metadata))
	...
	def _create_profile(self, ebuilds, eclasses, installed, profile, repo_configs, user_config, sets):
		...
		make_conf = {
			...
			"PKGDIR": self.pkgdir,
			"PORTAGE_INST_GID": str(pkgwh.data.portage_gid),
			"PORTAGE_INST_UID": str(pkgwh.data.portage_uid),
		...
		if not pkgwh.process.sandbox_capable or \
		...
		#Create /usr/share/portage/config/sets/pkgwh.conf
		...
		provided_sets_portage_conf = (
			os.path.join(cnf_path, "sets", "pkgwh.conf"))
		os.symlink(provided_sets_portage_conf, os.path.join(default_sets_conf_dir, "pkgwh.conf"))
	...
	def _load_config(self):
		...
		trees = pkgwh.create_trees(env=env, eprefix=self.eprefix,
			**create_trees_kwargs)
	...
	def run(self, atoms, options={}, action=None):
		...
		global_noiselimit = pkgwh.util.noiselimit
		...
		try:
			...
			pkgwh.util.noiselimit = global_noiselimit
