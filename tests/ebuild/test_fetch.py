...
import pkgwh
from pkgwh import shutil, os
from pkgwh.const import BASH_BINARY, PORTAGE_PYM_PATH
from pkgwh.tests import TestCase
from pkgwh.tests.resolver.ResolverPlayground import ResolverPlayground
from pkgwh.tests.util.test_socks5 import AsyncHTTPServer
from pkgwh.util.configparser import ConfigParserError
...
from pkgwh.package.ebuild.fetch import (_download_suffix, fetch, FlatLayout,
...
class EbuildFetchTestCase(TestCase):
	...
	def testEbuildFetch(self):
		...
		with AsyncHTTPServer(host, content, loop) as server:
			...
			try:
				fetchcommand = pkgwh.util.shlex_split(playground.settings['FETCHCOMMAND'])
				fetch_bin = pkgwh.process.find_binary(fetchcommand[0])
				...
				resumecommand = pkgwh.util.shlex_split(playground.settings['RESUMECOMMAND'])
				resume_bin = pkgwh.process.find_binary(resumecommand[0])
				...
				emirrordist_cmd = (pkgwh._python_interpreter, '-b', '-Wd',
				...
				for cpv in ebuilds:
					...
					pkgwh.doebuild_environment(ebuild_path, 'fetch', settings=settings, db=portdb)
