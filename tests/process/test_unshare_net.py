...
class UnshareNetTestCase(TestCase):
	...
	def testUnshareNet(self):
		...
		if pkgwh.process.find_binary('ping') is None:
			self.skipTest('ping not found')

		errno_value = pkgwh.process._unshare_validate(CLONE_NEWNET)
		if errno_value != 0:
			self.skipTest("Unable to unshare: %s" % (
				errno.errorcode.get(errno_value, '?')))

		env = os.environ.copy()
		env['IPV6'] = '1' if pkgwh.process._has_ipv6() else ''
		self.assertEqual(pkgwh.process.spawn([BASH_BINARY, '-c', UNSHARE_NET_TEST_SCRIPT], unshare_net=True, env=env), 0)
