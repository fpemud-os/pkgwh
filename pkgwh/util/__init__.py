...
from pkgwh.exception import InvalidAtom, PortageException, FileNotFound, \
	IsADirectory, OperationNotPermitted, ParseError, PermissionDenied, \
	ReadOnlyFileSystem
from pkgwh.localization import _
from pkgwh.proxy.objectproxy import ObjectProxy
from pkgwh.cache.mappings import UserDict
...
def stack_dictlist(original_dicts, incremental=0, incrementals=[], ignore_none=0):
	...
	Example usage:
	   >>> from pkgwh.util import stack_dictlist
...
class _getconfig_shlex(shlex.shlex):
	...
	def allow_sourcing(self, var_expand_map):
		self.source = pkgwh._native_string("source")
...
def getconfig(mycfg, tolerant=False, allow_sourcing=False, expand=True,
	...
	if content and content[-1] != pkgwh._native_string('\n'):
		content += pkgwh._native_string('\n')

	# Warn about dos-style line endings since that prevents
	# people from being able to source them with bash.
	if pkgwh._native_string('\r') in content:
	...
	try:
		...
		lex.wordchars = pkgwh._native_string(string.digits +
			string.ascii_letters + r"~!@#$%*_\:;?,./-+{}")
		lex.quotes = pkgwh._native_string("\"'")
		if allow_sourcing:
...
def unique_array(s):
	...
	# blah.	 back to original pkgwh.unique_array
	u = []
	for x in s:
		if x not in u:
			u.append(x)
	...
	if	(uid != -1 and uid != stat_cached.st_uid) or \
		...
		try:
			if follow_links:
				...
				pkgwh.data.lchown(filename, uid, gid)
			modified = True
		except OSError as oe:
	...
	for dirpath, dirnames, filenames in os.walk(top):
		for name, mode, mask in chain(
			...
			try:
				...
				if not isinstance(e, pkgwh.exception.InvalidLocation):
					all_applied = False
					onerror(e)
	return all_applied
...
def apply_secpass_permissions(filename, uid=-1, gid=-1, mode=-1, mask=-1,
	...
	# Avoid accessing pkgwh.data.secpass when possible, since
	# it triggers config loading (undesirable for chmod-lite).
	if (uid != -1 or gid != -1) and pkgwh.data.secpass < 2:
...
class atomic_ofstream(AbstractContextManager, ObjectProxy):
	...
	def __init__(self, filename, mode='w', follow_links=True, **kargs):
		...
		if follow_links:
			...
			tmp_name = "%s.%i" % (canonical_path, pkgwh.getpid())
			try:
		...
		tmp_name = "%s.%i" % (filename, pkgwh.getpid())
		object.__setattr__(self, '_file',
			open_func(_unicode_encode(tmp_name,
				encoding=_encodings['fs'], errors='strict'),
				mode=mode, **kargs))
	...
	if last_pfile and newmd5:
		try:
			...
			if stat.S_ISLNK(old_pfile_st.st_mode):
				...
				try:
					last_pfile_md5 = \
						pkgwh.checksum._perform_md5_merge(old_pfile)
				except FileNotFound:
