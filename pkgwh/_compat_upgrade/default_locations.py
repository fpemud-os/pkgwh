...
from pkgwh.const import GLOBAL_CONFIG_PATH
...
def main():
	...
	pkg_preinst() {
		...
		env -u DISTDIR \
			...
			"${PYTHON}" -m pkgwh._compat_upgrade.default_locations || die
	}
	"""
	out = pkgwh.output.EOutput()
	config = pkgwh.settings

	compat_distdir = os.path.join(pkgwh.const.EPREFIX or '/', COMPAT_DISTDIR)
	try:
		do_distdir = os.path.samefile(config['DISTDIR'], compat_distdir)
	except OSError:
		do_distdir = False

	compat_pkgdir = os.path.join(pkgwh.const.EPREFIX or '/', COMPAT_PKGDIR)
	try:
		do_pkgdir = os.path.samefile(config['PKGDIR'], compat_pkgdir)
	except OSError:
		do_pkgdir = False

	compat_rpmdir = os.path.join(pkgwh.const.EPREFIX or '/', COMPAT_RPMDIR)
	try:
		do_rpmdir = os.path.samefile(config['RPMDIR'], compat_rpmdir)
	except OSError:
		do_rpmdir = False

	compat_main_repo = os.path.join(pkgwh.const.EPREFIX or '/', COMPAT_MAIN_REPO)
	try:
