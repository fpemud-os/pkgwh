...
pkgwh.proxy.lazyimport.lazyimport(globals(),
	...
	'pkgwh.repository.config:_find_invalid_path_char',
	'pkgwh.util:write_atomic,writemsg_level',
)
...
from pkgwh.const import (MANIFEST2_HASH_DEFAULTS, MANIFEST2_IDENTIFIERS)
...
class Manifest:
	...
	def __init__(self, pkgdir, distdir=None, fetchlist_dict=None,
		...
		if manifest1_compat is not DeprecationWarning:
			warnings.warn("The manifest1_compat parameter of the "
				"pkgwh.manifest.Manifest constructor is deprecated.",
	...
	def _is_cpv(self, cat, pn, filename):
		if not filename.endswith(".ebuild"):
			return None
		pf = filename[:-7]
		ps = pkgwh.versions._pkgsplit(pf)
		cpv = "%s/%s" % (cat, pf)
		if not ps:
	...
	def _catsplit(self, pkg_key):
		...
		This is compatible with pkgwh.catsplit()"""
