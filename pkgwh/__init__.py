...
try:
	...
	import pkgwh.proxy.lazyimport
	import pkgwh.proxy as proxy
	proxy.lazyimport.lazyimport(globals(),
		'pkgwh.cache.cache_errors:CacheError',
		'pkgwh.checksum',
		'pkgwh.checksum:perform_checksum,perform_md5,prelink_capable',
		'pkgwh.cvstree',
		'pkgwh.data',
		'pkgwh.data:lchown,ostype,portage_gid,portage_uid,secpass,' + \
			'uid,userland,userpriv_groups,wheelgid',
		'pkgwh.dbapi',
		'pkgwh.dbapi.bintree:bindbapi,binarytree',
		'pkgwh.dbapi.cpv_expand:cpv_expand',
		'pkgwh.dbapi.dep_expand:dep_expand',
		'pkgwh.dbapi.porttree:close_portdbapi_caches,FetchlistDict,' + \
			'portagetree,portdbapi',
		'pkgwh.dbapi.vartree:dblink,merge,unmerge,vardbapi,vartree',
		'pkgwh.dbapi.virtual:fakedbapi',
		'pkgwh.dep',
		'pkgwh.dep:best_match_to_list,dep_getcpv,dep_getkey,' + \
			'flatten,get_operator,isjustname,isspecific,isvalidatom,' + \
			'match_from_list,match_to_list',
		'pkgwh.dep.dep_check:dep_check,dep_eval,dep_wordreduce,dep_zapdeps',
		'pkgwh.eclass_cache',
		'pkgwh.elog',
		'pkgwh.exception',
		'pkgwh.getbinpkg',
		'pkgwh.locks',
		'pkgwh.locks:lockdir,lockfile,unlockdir,unlockfile',
		'pkgwh.mail',
		'pkgwh.manifest:Manifest',
		'pkgwh.output',
		'pkgwh.output:bold,colorize',
		'pkgwh.package.ebuild.doebuild:doebuild,' + \
			'doebuild_environment,spawn,spawnebuild',
		'pkgwh.package.ebuild.config:autouse,best_from_dict,' + \
			'check_config_instance,config',
		'pkgwh.package.ebuild.deprecated_profile_check:' + \
			'deprecated_profile_check',
		'pkgwh.package.ebuild.digestcheck:digestcheck',
		'pkgwh.package.ebuild.digestgen:digestgen',
		'pkgwh.package.ebuild.fetch:fetch',
		'pkgwh.package.ebuild.getmaskingreason:getmaskingreason',
		'pkgwh.package.ebuild.getmaskingstatus:getmaskingstatus',
		'pkgwh.package.ebuild.prepare_build_dirs:prepare_build_dirs',
		'pkgwh.process',
		'pkgwh.process:atexit_register,run_exitfuncs',
		'pkgwh.update:dep_transform,fixdbentries,grab_updates,' + \
			'parse_updates,update_config_files,update_dbentries,' + \
			'update_dbentry',
		'pkgwh.util',
		'pkgwh.util:atomic_ofstream,apply_secpass_permissions,' + \
		...
		'pkgwh.util.listdir:cacheddir,listdir',
		'pkgwh.util.movefile:movefile',
		'pkgwh.util.mtimedb:MtimeDB',
		'pkgwh.versions',
		'pkgwh.versions:best,catpkgsplit,catsplit,cpv_getkey,' + \
	...
	import pkgwh.const
	from pkgwh.const import VDB_PATH, PRIVATE_PATH, CACHE_PATH, DEPCACHE_PATH, \
	...
	sys.stderr.write("!!! installation of pkgwh. Please try a rescue portage located in the ebuild\n")
	sys.stderr.write("!!! repository under '/var/db/repos/gentoo/sys-apps/portage/files/' (default).\n")
	sys.stderr.write("!!! There is a README.RESCUE file that details the steps required to perform\n")
	sys.stderr.write("!!! a recovery of pkgwh.\n")
	sys.stderr.write("    "+str(e)+"\n\n")
	raise
...
#      file names when the source encoding is unknown by pkgwh.
#
...
_supported_eapis = frozenset([str(x) for x in range(pkgwh.const.EAPI + 1)] + list(_testing_eapis) + list(_deprecated_eapis))
...
def _movefile(src, dest, **kwargs):
	...
	if movefile(src, dest, **kwargs) is None:
		raise pkgwh.exception.PortageException(
			"mv '%s' '%s'" % (src, dest))
...
def create_trees(config_root=None, target_root=None, trees=None, env=None,
	...
	for myroot, mysettings in myroots:
		trees[myroot] = pkgwh.util.LazyItemsDict(trees.get(myroot, {}))
...
class _LegacyGlobalProxy(proxy.objectproxy.ObjectProxy):
	...
	def _get_target(self):
		...
		from pkgwh._legacy_globals import _get_legacy_global
