...
from pkgwh.util._async.FileCopier import FileCopier
from pkgwh.util._async.FileDigester import FileDigester
from pkgwh.util._async.PipeLogger import PipeLogger
from pkgwh.util._async.PopenProcess import PopenProcess
from _emerge.CompositeTask import CompositeTask

default_hash_name = pkgwh.const.MANIFEST2_HASH_DEFAULT
...
class FetchTask(CompositeTask):
	...
	def _fetch_uri(self, uri):
		...
		tmp_basename = self.distfile + '._emirrordist_fetch_.%s' % pkgwh.getpid()
		...
		args = pkgwh.util.shlex_split(default_fetchcommand)
		args = [pkgwh.util.varexpand(x, mydict=variables)
			for x in args]
	...
	def _select_hash(self):
		...
		for hash_name in self.digests:
			if hash_name != "size" and \
				hash_name in pkgwh.checksum.get_valid_checksum_keys():
				return hash_name
