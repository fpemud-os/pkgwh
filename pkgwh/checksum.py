...
from pkgwh.const import HASHING_BLOCKSIZE, PRELINK_BINARY
...
def _open_file(filename):
	try:
		...
		if e.errno == errno.EPERM:
			raise pkgwh.exception.OperationNotPermitted(func_call)
		elif e.errno == errno.EACCES:
			raise pkgwh.exception.PermissionDenied(func_call)
		elif e.errno == errno.ENOENT:
			raise pkgwh.exception.FileNotFound(filename)
		else:
			raise
...
if "WHIRLPOOL" not in hashfunc_map:
	...
	from pkgwh.util.whirlpool import new as _new_whirlpool
	...
	try:
		if (calc_prelink and prelink_capable and
			...
			try:
				...
				try:
					retval = pkgwh.process.spawn([PRELINK_BINARY,
		...
		try:
			if hashname not in hashfunc_keys:
				raise pkgwh.exception.DigestException(hashname + \
					" hash function not available (needs dev-python/pycrypto)")
			myhash, mysize = hashfunc_map[hashname].checksum_file(myfilename)
		except (OSError, IOError) as e:
			if e.errno in (errno.ENOENT, errno.ESTALE):
				raise pkgwh.exception.FileNotFound(myfilename)
			elif e.errno == pkgwh.exception.PermissionDenied.errno:
				raise pkgwh.exception.PermissionDenied(myfilename)
			raise
...
def perform_multiple_checksums(filename, hashes=["MD5"], calc_prelink=0):
	...
	for x in hashes:
		if x not in hashfunc_keys:
			raise pkgwh.exception.DigestException(x+" hash function not available (needs dev-python/pycrypto or >=dev-lang/python-2.5)")
