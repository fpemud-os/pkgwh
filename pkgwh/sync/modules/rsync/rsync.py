...
from pkgwh.const import VCS_DIRS, TIMESTAMP_FORMAT, RSYNC_PACKAGE_ATOM
from pkgwh.output import create_color_func, yellow, blue, bold
from pkgwh.sync.getaddrinfo_validate import getaddrinfo_validate
from pkgwh.sync.syncbase import NewBase
from pkgwh.util import writemsg, writemsg_level, writemsg_stdout
...
class RsyncSync(NewBase):
	...
	def update(self):
		...
		quiet = '--quiet' in opts
		out = pkgwh.output.EOutput(quiet=quiet)
		...
		if self.repo.module_specific_options.get('sync-rsync-extra-opts'):
			self.extra_rsync_opts.extend(pkgwh.util.shlex_split(
		...
		try:
			...
			content = pkgwh.util.grabfile(self.servertimestampfile)
			timestamp = 0
			if content:
				try:
	...
	def retrieve_head(self, **kwargs):
		...
		if kwargs:
			self._kwargs(kwargs)
		last_sync = pkgwh.grabfile(os.path.join(self.repo.location, "metadata", "timestamp.commit"))
		ret = (1, False)
		if last_sync:
			try:
	...
	def _set_rsync_defaults(self):
		pkgwh.writemsg("PORTAGE_RSYNC_OPTS empty or unset, using hardcoded defaults\n")
		rsync_opts = [
	...
	def _validate_rsync_opts(self, rsync_opts, syncuri):
		...
		pkgwh.writemsg("Using PORTAGE_RSYNC_OPTS instead of hardcoded defaults\n", 1)
		rsync_opts.extend(pkgwh.util.shlex_split(
			self.settings.get("PORTAGE_RSYNC_OPTS", "")))
		for opt in ("--recursive", "--times"):
			if opt not in rsync_opts:
				pkgwh.writemsg(yellow("WARNING:") + " adding required option " + \
	...
	def _do_rsync(self, syncuri, timestamp, opts):
		...
		if self.usersync_uid is not None:
			...
			ensure_dirs_kwargs = {}
			if pkgwh.secpass >= 1:
				ensure_dirs_kwargs['gid'] = pkgwh.portage_gid
				ensure_dirs_kwargs['mode'] = 0o70
				ensure_dirs_kwargs['mask'] = 0
			pkgwh.util.ensure_dirs(tmpdir, **ensure_dirs_kwargs)
		else:
		...
		os.close(fd)
		if self.usersync_uid is not None:
			pkgwh.util.apply_permissions(tmpservertimestampfile,
		...
		try:
			...
			try:
				if self.rsync_initial_timeout:
					pkgwh.exception.AlarmSignal.register(
						self.rsync_initial_timeout)

				pids.extend(pkgwh.process.spawn(
				...
				if self.usersync_uid is not None:
					pkgwh.util.apply_permissions(tmpservertimestampfile,
						uid=os.getuid())
				content = pkgwh.grabfile(tmpservertimestampfile)
			finally:
				if self.rsync_initial_timeout:
					pkgwh.exception.AlarmSignal.unregister()
				try:
					os.unlink(tmpservertimestampfile)
				except OSError:
					pass
		except pkgwh.exception.AlarmSignal:
			# timed out
			print('timed out')
		...
		if exitcode == os.EX_OK:
			if (servertimestamp != 0) and (servertimestamp == timestamp):
				...
				exitcode = None
				try:
					exitcode = pkgwh.process.spawn(command,
						**self.spawn_kwargs)
				finally:
					if exitcode is None:
						# interrupted
