...
import pkgwh
from pkgwh.repository.storage.interface import RepoStorageException
from pkgwh.util import writemsg_level
from pkgwh.util._eventloop.global_event_loop import global_event_loop
from pkgwh.util.backoff import RandomExponentialBackoff
from pkgwh.util.futures._sync_decorator import _sync_methods
from pkgwh.util.futures.retry import retry
...
class SyncBase:
	...
	def __init__(self, bin_command, bin_pkg):
		...
		if bin_command:
			self.bin_command = pkgwh.process.find_binary(bin_command)
	...
	def _select_storage_module(self):
		...
		if self.repo.sync_rcu:
			mod_name = 'pkgwh.repository.storage.hardlink_rcu.HardlinkRcuRepoStorage'
		elif self.repo.sync_allow_hardlinks:
			mod_name = 'pkgwh.repository.storage.hardlink_quarantine.HardlinkQuarantineRepoStorage'
		else:
			mod_name = 'pkgwh.repository.storage.inplace.InplaceRepoStorage'
		return mod_name
	...
	def repo_storage(self):
		...
		if self._repo_storage is None:
			storage_cls = pkgwh.load_mod(self._select_storage_module())
	...
	def _refresh_keys(self, openpgp_env):
		...
		"""
		out = pkgwh.output.EOutput(quiet=('--quiet' in self.options['emerge_config'].opts))
