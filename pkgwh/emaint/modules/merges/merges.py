...
import pkgwh
from pkgwh import os, _unicode_encode
from pkgwh.const import MERGING_IDENTIFIER, EPREFIX, PRIVATE_PATH, VDB_PATH
...
class TrackingFile:
	...
	def save(self, failed_pkgs):
		...
		pkgwh.util.write_atomic(tracking_path, '\n'.join(lines))
...
class MergesHandler:
	...
	def _get_pkg_atoms(self, failed_pkgs, pkg_atoms, pkg_invalid_entries):
		...
		portdb = pkgwh.db[pkgwh.root]['porttree'].dbapi
		for failed_pkg in failed_pkgs:
			# validate pkg name
	...
	def fix(self, **kwargs):
		...
		vardb = pkgwh.db[pkgwh.root]['vartree'].dbapi
		still_failed_pkgs = {}
		for pkg, mtime in failed_pkgs.items():
