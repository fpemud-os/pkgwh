...
from pkgwh import _unicode_decode, _unicode_encode
	...
	if attachments == None:
		...
		for x in attachments:
			if isinstance(x, BaseMessage):
				...
				raise pkgwh.exception.PortageException(_("Can't handle type of attachment: %s") % type(x))
		...
		try:
			if int(mymailport) > 100000:
				...
				if not myconn.has_extn("STARTTLS"):
					raise pkgwh.exception.PortageException(_("!!! TLS support requested for logmail but not supported by server"))
				myconn.starttls()
				myconn.ehlo()
			else:
			...
			raise pkgwh.exception.PortageException(_("!!! An error occurred while trying to send logmail:\n")+str(e))
		except socket.error as e:
			raise pkgwh.exception.PortageException(_("!!! A network error occurred while trying to send logmail:\n%s\nSure you configured PORTAGE_ELOG_MAILURI correctly?") % str(e))
