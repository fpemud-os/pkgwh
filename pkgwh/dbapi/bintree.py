...
pkgwh.proxy.lazyimport.lazyimport(globals(),
	...
	'pkgwh.checksum:get_valid_checksum_keys,perform_multiple_checksums,' + \
	...
	'pkgwh.output:EOutput,colorize',
	'pkgwh.locks:lockfile,unlockfile',
	'pkgwh.package.ebuild.fetch:_check_distfile,_hide_url_passwd',
	'pkgwh.update:update_dbentries',
	'pkgwh.util:atomic_ofstream,ensure_dirs,normalize_path,' + \
		'writemsg,writemsg_stdout',
	'pkgwh.util.path:first_existing',
	'pkgwh.util._async.SchedulerInterface:SchedulerInterface',
	'pkgwh.util._urlopen:urlopen@_urlopen,have_pep_476@_have_pep_476',
	'pkgwh.versions:best,catpkgsplit,catsplit,_pkg_str',
)

from pkgwh.binrepo.config import BinRepoConfigLoader
from pkgwh.cache.mappings import slot_dict_class
from pkgwh.const import BINREPOS_CONF_FILE, CACHE_PATH, SUPPORTED_XPAK_EXTENSIONS
from pkgwh.dbapi.virtual import fakedbapi
from pkgwh.dep import Atom, use_reduce, paren_enclose
from pkgwh.exception import AlarmSignal, InvalidData, InvalidPackageName, \
	ParseError, PortageException
from pkgwh.localization import _
from pkgwh.package.ebuild.profile_iuse import iter_iuse_vars
...
class bindbapi(fakedbapi):
	...
	def aux_get(self, mycpv, wants, myrepo=None):
		...
		if not self.bintree._remotepkgs or \
			...
			try:
				...
				raise KeyError(mycpv)
			metadata_bytes = pkgwh.xpak.tbz2(tbz2_path).get_data()
			def getitem(k):
				if k == "_mtime_":
	...
	def aux_update(self, cpv, values):
		...
		if not os.path.exists(tbz2path):
			raise KeyError(cpv)
		mytbz2 = pkgwh.xpak.tbz2(tbz2path)
		...
		for k, v in list(mydata.items()):
			if not v:
				del mydata[k]
		mytbz2.recompose_mem(pkgwh.xpak.xpak_mem(mydata))
	...
	def unpack_metadata(self, pkg, dest_dir, loop=None):
		...
		if add_pkg is not None:
			...
			yield loop.run_in_executor(ForkExecutor(loop=loop),
				pkgwh.xpak.tbz2(tbz2_file).unpackinfo, dest_dir)
	...
	def getfetchsizes(self, pkg):
		...
		if not self.bintree.isremote(pkg):
			...
			try:
				...
				raise pkgwh.exception.MissingSignature("SIZE")
			except ValueError:
				raise pkgwh.exception.InvalidSignature(
...
class binarytree:
	...
	def __init__(self, _unused=DeprecationWarning, pkgdir=None,
		...
		if _unused is not DeprecationWarning:
			warnings.warn("The first parameter of the "
				"pkgwh.dbapi.bintree.binarytree"
	...
	def move_ent(self, mylist, repo_match=None):
		...
		for mycpv in origmatches:
			try:
				...
				continue
			mycpv_cp = pkgwh.cpv_getkey(mycpv)
			if mycpv_cp != origcp:
	...
	def prevent_collision(self, cpv):
		warnings.warn("The "
			"pkgwh.dbapi.bintree.binarytree.prevent_collision "
	...
	def _file_permissions(self, path):
		try:
			...
			try:
				pkgwh.util.apply_permissions(path, gid=pkgdir_gid,
			...
			try:
				...
				if f is None:
					...
					if repo.fetchcommand is None and parsed_url.scheme == 'ssh':
						...
						ssh_args.extend(pkgwh.util.shlex_split(
						...
						success = pkgwh.getbinpkg.file_get(
		...
		try:
			...
			if (build_id is None and not fetched and
				...
				cpv = _pkg_str(cpv, metadata=metadata,
					settings=self.settings, db=self.dbapi)
				binpkg = pkgwh.xpak.tbz2(full_path)
				binary_data = binpkg.get_data()
				binary_data[b"BUILD_ID"] = _unicode_encode(
					metadata["BUILD_ID"])
				binpkg.recompose_mem(pkgwh.xpak.xpak_mem(binary_data))
	...
	def _read_metadata(self, filename, st, keys=None):
		...
		if keys is None:
			...
			metadata = {}
		binary_metadata = pkgwh.xpak.tbz2(filename).get_data()
		for k in keys:
			if k == "_mtime_":
	...
	def _new_pkgindex(self):
		return pkgwh.getbinpkg.PackageIndex(
	...
	def gettbz2(self, pkgname):
		...
		success = pkgwh.getbinpkg.file_get(url, mydest, fcmd=fcmd)
		if not success:
			try:
				os.unlink(self.getname(pkgname))
			except OSError:
				pass
			raise pkgwh.exception.FileNotFound(mydest)
		self.inject(pkgname)
	...
	def digestCheck(self, pkg):
		...
		if not ok:
			...
			if not ok:
				raise pkgwh.exception.DigestException(
