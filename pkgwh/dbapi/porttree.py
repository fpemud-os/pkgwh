...
pkgwh.proxy.lazyimport.lazyimport(globals(),
	'pkgwh.checksum',
	'pkgwh.data:portage_gid,secpass',
	'pkgwh.dbapi.dep_expand:dep_expand',
	'pkgwh.dep:Atom,dep_getkey,match_from_list,use_reduce,_match_slot',
	'pkgwh.package.ebuild.doebuild:doebuild',
	'pkgwh.package.ebuild.fetch:_download_suffix',
	'pkgwh.util:ensure_dirs,shlex_split,writemsg,writemsg_level',
	'pkgwh.util.listdir:listdir',
	'pkgwh.versions:best,catsplit,catpkgsplit,_pkgsplit@pkgsplit,ver_regexp,_pkg_str',
)

from pkgwh.cache import volatile
from pkgwh.cache.cache_errors import CacheError
from pkgwh.cache.mappings import Mapping
from pkgwh.dbapi import dbapi
from pkgwh.exception import pkgwhException, PortageKeyError, \
	FileNotFound, InvalidAtom, InvalidData, \
	InvalidDependString, InvalidPackageName
from pkgwh.localization import _

from pkgwh import eclass_cache, \
	eapi_is_supported, \
	_eapi_is_deprecated
...
from pkgwh.util.futures.iter_completed import iter_gather
...
def close_portdbapi_caches():
	...
	try:
		pkgwh._legacy_globals_constructed
	except AttributeError:
		pass
	else:
		if "db" in pkgwh._legacy_globals_constructed:
			try:
				db = pkgwh.db
			except AttributeError:
				pass
...
pkgwh.process.atexit_register(close_portdbapi_caches)
...
class portdbapi(dbapi):
	...
	def porttree_root(self):
		warnings.warn("pkgwh.dbapi.porttree.portdbapi.porttree_root is deprecated in favor of pkgwh.repository.config.RepoConfig.location "
			"(available as repositories[repo_name].location attribute of instances of pkgwh.dbapi.porttree.portdbapi class)",
			DeprecationWarning, stacklevel=2)
		return self.settings.repositories.mainRepoLocation()

	@property
	def eclassdb(self):
		warnings.warn("pkgwh.dbapi.porttree.portdbapi.eclassdb is deprecated in favor of pkgwh.repository.config.RepoConfig.eclass_db "
			"(available as repositories[repo_name].eclass_db attribute of instances of pkgwh.dbapi.porttree.portdbapi class)",
			DeprecationWarning, stacklevel=2)
	...
	def __init__(self, _unused_param=DeprecationWarning, mysettings=None):
		...
		@type mysettings: pkgwh.config
		"""

		from pkgwh import config
		if mysettings:
			self.settings = mysettings
		else:
			from pkgwh import settings
			self.settings = config(clone=settings)

		if _unused_param is not DeprecationWarning:
			warnings.warn("The first parameter of the " + \
				"pkgwh.dbapi.porttree.portdbapi" + \
	...
	def _set_porttrees(self, porttrees):
		...
		@type porttrees: list
		"""
		self._porttrees_repos = pkgwh.OrderedDict((repo.name, repo)
		...
		def aux_get_done(aux_get_future):
			...
			if aux_get_future.exception() is not None:
				if isinstance(aux_get_future.exception(), PortageKeyError):
					...
					result.set_exception(pkgwh.exception.InvalidDependString(
	...
	def fetch_check(self, mypkg, useflags=None, mysettings=None, all=False, myrepo=None): # pylint: disable=redefined-builtin
		...
		for x in myfiles:
			if not mysums or x not in mysums:
				...
				try:
					ok, reason = pkgwh.checksum.verify_all(
	...
	def xmatch(self, level, origdep, mydep=DeprecationWarning,
		...
		@type origdep: pkgwh.dep.Atom or str
		...
		if level == "list-visible":
			...
			warnings.warn("The 'list-visible' mode of "
				"pkgwh.dbapi.porttree.portdbapi.xmatch "
...
class portagetree:
	def __init__(self, root=DeprecationWarning, virtual=DeprecationWarning,
		...
		@param settings: Portage Configuration object (pkgwh.settings)
		...
		if root is not DeprecationWarning:
			warnings.warn("The root parameter of the " + \
				"pkgwh.dbapi.porttree.portagetree" + \
	...
	def portroot(self):
		...
		warnings.warn("The portroot attribute of "
			"pkgwh.dbapi.porttree.portagetree is deprecated. Use the "
	...
	def getname(self, pkgname):
		...
		warnings.warn("The getname method of "
			"pkgwh.dbapi.porttree.portagetree is deprecated. "
...
class FetchlistDict(Mapping):
	...
	to allow pkgwh.manifest.Manifest to access fetch lists via a standard
	...
	def has_key(self, pkg_key):
		...
		warnings.warn("pkgwh.dbapi.porttree.FetchlistDict.has_key() is "
...
def _async_manifest_fetchlist(portdb, repo_config, cp, cpv_list=None,
	...
	@param max_jobs: max number of futures to process concurrently (default
		is pkgwh.util.cpuinfo.get_cpu_count())
	@type max_jobs: int
...
def _parse_uri_map(cpv, metadata, use=None):
	...
	while myuris:
		...
		if myuris and myuris[-1] == "->":
			...
			if not distfile:
				raise pkgwh.exception.InvalidDependString(
