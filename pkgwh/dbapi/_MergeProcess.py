...
import fcntl
import pkgwh
from pkgwh import os, _unicode_decode
from pkgwh.util._ctypes import find_library
import pkgwh.elog.messages
...
class MergeProcess(ForkProcess):
	...
	def _start(self):
		...
		self.fd_pipes.setdefault(0, pkgwh._get_stdin().fileno())
		...
		if output:
			...
			if len(lines) == 1:
				...
				for line in lines:
					...
					reporter = getattr(pkgwh.elog.messages, funcname)
	...
	def _spawn(self, args, fd_pipes, **kwargs):
		...
		mylink = pkgwh.dblink(self.mycat, self.mypkg, settings=self.settings,
		...
		pkgwh.elog.messages.collect_messages(key=mylink.mycpv)
	...
	def _run(self):
			...
			pkgwh.output.havecolor = self.settings.get('NOCOLOR') \
				not in ('yes', 'true')
	...
	def _proc_join_done(self, proc, future):
		...
		"""
		if not future.cancelled() and proc.exitcode == pkgwh.const.RETURNCODE_POSTINST_FAILURE:
	...
	def _unregister(self):
		...
		if self._elog_keys is not None:
			for key in self._elog_keys:
				pkgwh.elog.elog_process(key, self.settings,
