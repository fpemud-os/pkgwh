...
pkgwh.proxy.lazyimport.lazyimport(globals(),
	'hashlib:md5',
	'pkgwh.checksum:_perform_md5_merge@perform_md5',
	'pkgwh.data:portage_gid,portage_uid,secpass',
	'pkgwh.dbapi.dep_expand:dep_expand',
	'pkgwh.dbapi._MergeProcess:MergeProcess',
	'pkgwh.dbapi._SyncfsProcess:SyncfsProcess',
	'pkgwh.dep:dep_getkey,isjustname,isvalidatom,match_from_list,' + \
	 	'use_reduce,_slot_separator,_repo_separator',
	'pkgwh.eapi:_get_eapi_attrs',
	'pkgwh.elog:collect_ebuild_messages,collect_messages,' + \
		'elog_process,_merge_logentries',
	'pkgwh.locks:lockdir,unlockdir,lockfile,unlockfile',
	'pkgwh.output:bold,colorize',
	'pkgwh.package.ebuild.doebuild:doebuild_environment,' + \
		'_merge_unicode_error',
	'pkgwh.package.ebuild.prepare_build_dirs:prepare_build_dirs',
	'pkgwh.package.ebuild._ipc.QueryCommand:QueryCommand',
	'pkgwh.process:find_binary',
	'pkgwh.util:apply_secpass_permissions,ConfigProtect,ensure_dirs,' + \
		'writemsg,writemsg_level,write_atomic,atomic_ofstream,writedict,' + \
		'grabdict,normalize_path,new_protect_filename',
	'pkgwh.util._compare_files:compare_files',
	'pkgwh.util.digraph:digraph',
	'pkgwh.util.env_update:env_update',
	'pkgwh.util.install_mask:install_mask_dir,InstallMask,_raise_exc',
	'pkgwh.util.listdir:dircache,listdir',
	'pkgwh.util.movefile:movefile',
	'pkgwh.util.path:first_existing,iter_parents',
	'pkgwh.util.writeable_check:get_ro_checker',
	'pkgwh.util._xattr:xattr',
	'pkgwh.util._dyn_libs.PreservedLibsRegistry:PreservedLibsRegistry',
	'pkgwh.util._dyn_libs.LinkageMapELF:LinkageMapELF@LinkageMap',
	...
	'pkgwh.versions:best,catpkgsplit,catsplit,cpv_getkey,vercmp,' + \
		'_get_slot_re,_pkgsplit@pkgsplit,_pkg_str,_unknown_repo',
	'subprocess',
	'tarfile',
)

from pkgwh.const import CACHE_PATH, CONFIG_MEMORY_FILE, \
	MERGING_IDENTIFIER, PORTAGE_PACKAGE_ATOM, PRIVATE_PATH, VDB_PATH
from pkgwh.dbapi import dbapi
from pkgwh.exception import CommandNotFound, \
	InvalidData, InvalidLocation, InvalidPackageName, \
	FileNotFound, PermissionDenied, UnsupportedAPIException
from pkgwh.localization import _
from pkgwh.util.futures import asyncio

from pkgwh import abssymlink, _movefile, bsd_chflags

# This is a special version of the os module, wrapped for unicode support.
from pkgwh import os
from pkgwh import shutil
from pkgwh import _encodings
from pkgwh import _os_merge
from pkgwh import _selinux_merge
from pkgwh import _unicode_decode
from pkgwh import _unicode_encode
...
class vardbapi(dbapi):
	...
	def __init__(self, _unused_param=DeprecationWarning,
		...
		if vartree is None:
			vartree = pkgwh.db[settings['EROOT']]['vartree']
	...
	def cpv_all(self, use_cache=1):
		"""
		Set use_cache=0 to bypass the pkgwh.cachedir() cache in cases
	...
	def _aux_env_search(self, cpv, variables):
		...
		if not os.path.isfile(env_file):
			return {}
		bunzip2_cmd = pkgwh.util.shlex_split(
		...
		try:
			...
			if e.errno != errno.ENOENT:
				raise
			raise pkgwh.exception.CommandNotFound(args[0])
	...
	def unpack_metadata(self, pkg, dest_dir, loop=None):
		...
		@type pkg: _pkg_str or pkgwh.config
		@param dest_dir: destination directory
		@type dest_dir: str
		"""
		loop = asyncio._wrap_loop(loop)
		if not isinstance(pkg, pkgwh.config):
			cpv = pkg
		else:
			cpv = pkg.mycpv
	...
	def unpack_contents(self, pkg, dest_dir,
		...
		opts_list = pkgwh.util.shlex_split(settings.get('QUICKPKG_DEFAULT_OPTS', ''))
		...
		if excluded_config_files:
			...
			for line in log_lines:
				pkgwh.elog.messages.ewarn(line, phase='install', key=cpv, out=out)
...
class dblink:
	...
	def __init__(self, cat, pkg, myroot=None, settings=None, treetype=None,
		...
		@param settings: Typically pkgwh.settings
		@type settings: pkgwh.config
		...
		if vartree is None:
			vartree = pkgwh.db[self._eroot]["vartree"]
		self.vartree = vartree
	...
	def _acquire_slot_locks(self, db):
		...
		slot_atoms.append(pkgwh.dep.Atom(
	...
	def delete(self):
		...
		if not self.dbdir.startswith(self.dbroot):
			writemsg(_("pkgwh.dblink.delete(): invalid dbdir: %s\n") % \
	...
	def quickpkg(self, output_file, include_config=False, include_unmodified_config=False):
		...
		if not include_config:
			confprot = ConfigProtect(settings['EROOT'],
				pkgwh.util.shlex_split(settings.get('CONFIG_PROTECT', '')),
				pkgwh.util.shlex_split(settings.get('CONFIG_PROTECT_MASK', '')),
	...
	def unmerge(self, pkgfiles=None, trimworld=None, cleanup=True,
		...
		if trimworld is not None:
			warnings.warn("The trimworld parameter of the " + \
				"pkgwh.dbapi.vartree.dblink.unmerge()" + \
		...
		if others_in_slot is None:
			...
			slot_matches = self.vartree.dbapi.match(
				"%s:%s" % (pkgwh.cpv_getkey(self.mycpv), slot))
			others_in_slot = []
	...
	def _unmerge_pkgfiles(self, pkgfiles, others_in_slot):
		...
		if pkgfiles:
			...
			uninstall_ignore = pkgwh.util.shlex_split(
				...
				if os is _os_merge:
					try:
						...
						try:
							...
							os = pkgwh.os
							perf_md5 = pkgwh.checksum.perform_md5
	...
	def isowner(self, filename, destroot=None):
		...
		if destroot is not None and destroot != self._eroot:
			warnings.warn("The second parameter of the " + \
				"pkgwh.dbapi.vartree.dblink.isowner()" + \
	...
	def _match_contents(self, filename, destroot=None):
		...
		if destroot is not None and destroot != self._eroot:
			warnings.warn("The second parameter of the " + \
				"pkgwh.dbapi.vartree.dblink._match_contents()" + \
				" is now unused. Instead " + \
	...
	def _collision_protect(self, srcroot, destroot, mypkglist,
		file_list, symlink_list):
			...
			collision_ignore = []
			for x in pkgwh.util.shlex_split(
	...
	def _elog(self, funcname, phase, lines):
		func = getattr(pkgwh.elog.messages, funcname)
		if self._scheduler is None:
			for l in lines:
	...
	def treewalk(self, srcroot, destroot, inforoot, myebuild, cleanup=0,
		...
		try:
			...
			for cur_cpv in slot_matches:
				...
				# like elog.
				settings_clone = pkgwh.config(clone=self.settings)
		...
		self.settings["REPLACING_VERSIONS"] = " ".join(
			[pkgwh.versions.cpv_getversion(other.mycpv)
		...
		for dblnk in list(others_in_slot):
			...
			dblnk.settings["REPLACED_BY_VERSION"] = pkgwh.versions.cpv_getversion(self.mycpv)
				...
				if mydmode != None:
					...
					if stat.S_ISDIR(mydmode) or \
						...
						try:
							...
							# pkgwh.util.ensure_dirs() for cases
							# like bug #187518.
	...
	def merge(self, mergeroot, inforoot, myroot=None, myebuild=None, cleanup=0,
		...
		if retval == os.EX_OK and self._postinst_failure:
			retval = pkgwh.const.RETURNCODE_POSTINST_FAILURE
	...
	def _quickpkg_dblink(self, backup_dblink, background, logfile):
		...
		try:
			...
			if not pythonpath or \
				not os.path.samefile(pythonpath[0], pkgwh._pym_path):
				pythonpath.insert(0, pkgwh._pym_path)
			env['PYTHONPATH'] = ":".join(pythonpath)

			quickpkg_proc = SpawnProcess(
				args=[pkgwh._python_interpreter, quickpkg_binary,
