...
pkgwh.proxy.lazyimport.lazyimport(globals(),
	'pkgwh.data:portage_gid',
	'pkgwh.dep.soname.SonameAtom:SonameAtom',
	'pkgwh.dbapi.vartree:vartree',
	'pkgwh.package.ebuild.doebuild:_phase_func_map',
	'pkgwh.util.compression_probe:_compressors',
	'pkgwh.util.locale:check_locale,split_LC_ALL',
)
from pkgwh import bsd_chflags, \
	load_mod, os, selinux, _unicode_decode
from pkgwh.const import CACHE_PATH, \
...
from pkgwh.dep import Atom, isvalidatom, match_from_list, use_reduce, _repo_separator, _slot_separator
from pkgwh.eapi import (eapi_exports_AA, eapi_exports_merge_type,
	eapi_supports_prefix, eapi_exports_replace_vars, _get_eapi_attrs)
from pkgwh.env.loaders import KeyValuePairFileLoader
from pkgwh.exception import InvalidDependString, PortageException
from pkgwh.localization import _
from pkgwh.output import colorize
from pkgwh.process import fakeroot_capable, sandbox_capable
from pkgwh.repository.config import load_repository_config
from pkgwh.util import ensure_dirs, getconfig, grabdict, \
	grabdict_package, grabfile, grabfile_package, LazyItemsDict, \
	normalize_path, shlex_split, stack_dictlist, stack_dicts, stack_lists, \
	writemsg, writemsg_level, _eapi_cache
from pkgwh.util.install_mask import _raise_exc
from pkgwh.util.path import first_existing
from pkgwh.util._path import exists_raise_eaccess, isdir_raise_eaccess
from pkgwh.versions import catpkgsplit, catsplit, cpv_getkey, _pkg_str

from pkgwh.package.ebuild._config import special_env_vars
from pkgwh.package.ebuild._config.env_var_validation import validate_cmd_var
from pkgwh.package.ebuild._config.features_set import features_set
from pkgwh.package.ebuild._config.KeywordsManager import KeywordsManager
from pkgwh.package.ebuild._config.LicenseManager import LicenseManager
from pkgwh.package.ebuild._config.UseManager import UseManager
from pkgwh.package.ebuild._config.LocationsManager import LocationsManager
from pkgwh.package.ebuild._config.MaskManager import MaskManager
from pkgwh.package.ebuild._config.VirtualsManager import VirtualsManager
from pkgwh.package.ebuild._config.helper import ordered_by_atom_specificity, prune_incremental
from pkgwh.package.ebuild._config.unpack_dependencies import load_unpack_dependencies_configuration
...
def autouse(myvartree, use_cache=1, mysettings=None):
	warnings.warn("pkgwh.autouse() is deprecated",
...
class config:
	...
	_module_aliases = {
		"cache.metadata_overlay.database" : "pkgwh.cache.flat_hash.mtime_md5_database",
		"pkgwh.cache.metadata_overlay.database" : "pkgwh.cache.flat_hash.mtime_md5_database",
	}
	...
	def __init__(self, clone=None, mycpv=None, config_profile_path=None,
		...
		@type mycpv: String
		@param config_profile_path: Configurable path to the profile (usually PROFILE_PATH from pkgwh.const)
		@type config_profile_path: String
		@param config_incrementals: List of incremental variables
			(defaults to pkgwh.const.INCREMENTALS)
		...
		@type sysroot: String
		@param eprefix: set the EPREFIX variable (default is pkgwh.const.EPREFIX)
		@type eprefix: String
		...
		@param repositories: Configuration of repositories.
			Defaults to pkgwh.repository.config.load_repository_config().
		@type repositories: Instance of pkgwh.repository.config.RepoConfigLoader class.
		"""

		# This is important when config is reloaded after emerge --sync.
		_eapi_cache.clear()

		# When initializing the global pkgwh.settings instance, avoid
		# raising exceptions whenever possible since exceptions thrown
		# from 'import pkgwh' or 'import pkgwh.exceptions' statements
		...
		if clone:
			...
			self.modules["default"] = {
				"portdbapi.auxdbmodule":  "pkgwh.cache.flat_hash.mtime_md5_database",
			}
			...
			if portdir_overlay:
				for ov in portdir_overlay:
					...
					if isdir_raise_eaccess(ov) or pkgwh._sync_mode:
						new_ov.append(pkgwh._shell_quote(ov))
					else:
			...
			self["PORTAGE_OVERRIDE_EPREFIX"] = pkgwh.const.EPREFIX
			self.backup_changes("PORTAGE_OVERRIDE_EPREFIX")

			self._ppropertiesdict = pkgwh.dep.ExtendedAtomDict(dict)
			self._paccept_restrict = pkgwh.dep.ExtendedAtomDict(dict)
			self._penvdict = pkgwh.dep.ExtendedAtomDict(dict)
			self._pbashrcdict = {}
			self._pbashrc = ()
			...
			if local_config:
				...
				for profile in profiles_complex:
					...
					self._pbashrcdict[profile] = \
						pkgwh.dep.ExtendedAtomDict(dict)
			...
			try:
				...
				if pkgwh.data._unprivileged_mode(
			...
			if self.depcachedir is None:
				self.depcachedir = os.path.join(os.sep,
					pkgwh.const.EPREFIX, DEPCACHE_PATH.lstrip(os.sep))
			...
			pkgwh.output._init(config_root=self['PORTAGE_CONFIGROOT'])
			pkgwh.data._init(self)
	...
	@property
	def mygcfg(self):
		warnings.warn("pkgwh.config.mygcfg is deprecated", stacklevel=3)
		return {}
	...
	def validate(self):
		...
		(This code was previously in the global scope of pkgwh.py)"""
		...
		if profile_broken and not pkgwh._sync_mode:
			...
			try:
				...
				try:
					compression_binary = shlex_split(
						pkgwh.util.varexpand(compression["compress"],
						mydict=self))[0]
			...
			if pkgwh._internal_caller:
				...
				try:
					...
					with open("/dev/tty", "w") as f:
						f.write("=" * 96 + "\n")
						f.write("=" * 8 + " Traceback for invalid call to pkgwh.package.ebuild.config.config.__getitem__ " + "=" * 8 + "\n")
						f.writelines(stack)
						f.write("=" * 96 + "\n")
				except Exception:
					pass
				...
				warnings.warn(_("Passing nonexistent key %r to %s is deprecated. Use %s instead.") %
					(key, "pkgwh.package.ebuild.config.config.__getitem__",
					"pkgwh.package.ebuild.config.config.get"), DeprecationWarning, stacklevel=2)
				return ""
	...
	def _getitem(self, mykey):
		...
		if mykey in self._constant_keys:
			...
			if mykey == "PORTAGE_BIN_PATH":
				return pkgwh._bin_path
			if mykey == "PORTAGE_PYM_PATH":
				return pkgwh._pym_path
			...
			if mykey == "PORTAGE_PYTHONPATH":
				...
				if value:
					try:
						...
						pass
				if need_pym_path:
					value.insert(0, pkgwh._pym_path)
				return ":".join(value)
