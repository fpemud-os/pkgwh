# pkgwh.py -- core Portage functionality
...
pkgwh.proxy.lazyimport.lazyimport(globals(),
	'pkgwh.util:dump_traceback,writemsg',
)

from pkgwh.const import BASH_BINARY, SANDBOX_BINARY, FAKEROOT_BINARY
from pkgwh.exception import CommandNotFound
from pkgwh.util._ctypes import find_library, LoadLibrary, ctypes

try:
	from pkgwh.util.netlink import RtNetlink
except ImportError:
...
def run_exitfuncs():
	...
	while _exithandlers:
		...
		try:
			...
			dump_traceback("Error in pkgwh.process.run_exitfuncs", noiselevel=0)
			exc_info = sys.exc_info()
...
def spawn(mycommand, env=None, opt_name=None, fd_pipes=None, returnpid=False,
	...
	# Force instantiation of pkgwh.data.userpriv_groups before the
	# fork, so that the result is cached in the main process.
	bool(groups)

	parent_pid = pkgwh.getpid()
	pid = None
	try:
		pid = os.fork()
		...
		# Don't used pkgwh.getpid() here, due to a race with the above
		# pkgwh._ForkWatcher cache update.
...
def _exec(binary, mycommand, opt_name, fd_pipes,
	...
	if not opt_name:
		if binary is pkgwh._python_interpreter:
				...
				try:
					...
					if errno_value != 0:
						...
						if unshare_pid:
							...
							if main_child_pid == 0:
								# The pkgwh.getpid() cache may need to be updated here,
								# in case the pre_exec function invokes portage APIs.
								pkgwh._ForkWatcher.hook(pkgwh._ForkWatcher)
								# pid namespace requires us to become init
								binary, myargs = pkgwh._python_interpreter, [
									pkgwh._python_interpreter,
									os.path.join(pkgwh._bin_path,
										'pid-ns-init'),
...
def _setup_pipes(fd_pipes, close_fds=True, inheritable=None):
	...
	file descriptors, see pkgwh.locks._close_fds().
