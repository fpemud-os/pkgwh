...
class DoebuildProcess(ForkProcess):
	...
	def _run(self):
		return pkgwh.doebuild(*self.doebuild_pargs, **self.doebuild_kwargs)

class DoebuildFdPipesTestCase(TestCase):

	def testDoebuild(self):
		"""
		Invoke pkgwh.doebuild() with the fd_pipes parameter, and
		...
		true_binary = pkgwh.process.find_binary("true")
		...
		try:
			...
			settings = pkgwh.config(clone=playground.settings)
			...
			for phase in ('info', 'nofetch',
				...
				task_scheduler.wait()
				output = pkgwh._unicode_decode(
					consumer.getvalue()).rstrip("\n")

				if task_scheduler.returncode != os.EX_OK:
					pkgwh.writemsg(output, noiselevel=-1)
