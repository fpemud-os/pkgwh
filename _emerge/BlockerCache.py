...
class BlockerCache(pkgwh.cache.mappings.MutableMapping):
	...
	def __init__(self, myroot, vardb):
		...
		self._cache_filename = os.path.join(vardb.settings['EROOT'],
			pkgwh.CACHE_PATH, "vdb_blockers.pickle")
		...
		if cache_valid:
			...
			for k, v in self._cache_data["blockers"].items():
				if not isinstance(k, str):
					...
					continue
				try:
					if pkgwh.catpkgsplit(k) is None:
						invalid_items.add(k)
						continue
				except pkgwh.exception.InvalidData:
					invalid_items.add(k)
					continue
	...
	def flush(self):
		...
		if len(self._modified) >= self._cache_threshold and \
			secpass >= 2:
			try:
				f = pkgwh.util.atomic_ofstream(self._cache_filename, mode='wb')
				pickle.dump(self._cache_data, f, protocol=2)
				f.close()
				pkgwh.util.apply_secpass_permissions(
					self._cache_filename, gid=pkgwh.portage_gid, mode=0o644)
