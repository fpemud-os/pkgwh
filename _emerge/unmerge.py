...
from pkgwh.output import bold, colorize, darkgreen, green
from pkgwh._sets import SETPREFIX
from pkgwh._sets.base import EditablePackageSet
from pkgwh.versions import cpv_sort_key, _pkg_str
...
def _unmerge_display(root_config, myopts, unmerge_action,
	...
	writemsg_level=pkgwh.util.writemsg_level):
	"""
	...
	vdb_path = os.path.join(settings["EROOT"], pkgwh.VDB_PATH)
	try:
		# At least the parent needs to exist for the lock file.
		pkgwh.util.ensure_dirs(vdb_path)
	...
	try:
		...
		for x in candidate_catpkgs:
			...
			except pkgwh.exception.AmbiguousPackageName as errpkgs:
			...
			if unmerge_action in ["rage-clean", "unmerge"]:
				...
				for mypkg in mymatch[1:]:
					...
					if (myslot == best_slot and mycounter > best_counter) or \
						mypkg == pkgwh.best([mypkg, best_version]):
						if myslot == best_slot:
	...
	for cp in range(len(pkgmap)):
		for cpv in pkgmap[cp]["selected"].copy():
			...
			if unmerge_action != "clean" and root_config.root == "/":
				skip_pkg = False
				if pkgwh.match_from_list(pkgwh.const.PORTAGE_PACKAGE_ATOM,
						[pkg]):
			...
			for s in installed_sets:
				...
				if s not in sets:
					...
					out.eerror(("Unknown set '@%s' in %s%s") % \
						(s, root_config.settings['EROOT'], pkgwh.const.WORLD_SETS_FILE))
					continue
	...
	if not ordered:
		...
		for d in pkgmap:
			...
			if not selected:
				continue
			cp = pkgwh.cpv_getkey(next(iter(selected)))
...
def unmerge(root_config, myopts, unmerge_action,
	...
	scheduler=None, writemsg_level=pkgwh.util.writemsg_level):
	"""
	...
	for x in range(len(pkgmap)):
		for y in pkgmap[x]["selected"]:
			...
			mysplit = y.split("/")
			#unmerge...
			retval = pkgwh.unmerge(mysplit[0], mysplit[1],
				settings=mysettings,
