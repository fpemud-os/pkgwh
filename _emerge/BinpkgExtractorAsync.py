...
from pkgwh.util.compression_probe import (
	compression_probe,
	_compressors,
)
from pkgwh.process import find_binary
from pkgwh.util import (
	shlex_split,
	varexpand,
)
import signal
...
class BinpkgExtractorAsync(SpawnProcess):
	...
	_shell_binary = pkgwh.const.BASH_BINARY
	...
	def _start(self):
		...
		if "xattr" in self.features:
			...
			if b"--xattrs" in output:
				...
				for x in pkgwh.util.shlex_split(self.env.get("PORTAGE_XATTR_EXCLUDE", "")):
					tar_options.append(pkgwh._shell_quote("--xattrs-exclude=%s" % x))
				tar_options = " ".join(tar_options)

		decomp = _compressors.get(compression_probe(self.pkg_path))
		if decomp is not None:
			decomp_cmd = decomp.get("decompress")
		elif tarfile.is_tarfile(pkgwh._unicode_encode(self.pkg_path,
			encoding=pkgwh._encodings['fs'], errors='strict')):
			decomp_cmd = 'cat'
			decomp = {
				'compress': 'cat',
		...
		pkg_xpak = pkgwh.xpak.tbz2(self.pkg_path)
		pkg_xpak.scan()
		...
		self.args = [self._shell_binary, "-c",
			...
			pkgwh._shell_quote(self.pkg_path),
			decomp_cmd,
			tar_options,
			pkgwh._shell_quote(self.image_dir),
			128 + signal.SIGPIPE)]
