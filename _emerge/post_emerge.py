...
from pkgwh.emaint.modules.logs.logs import CleanLogs
from pkgwh.news import count_unread_news, display_news_notifications
from pkgwh.output import colorize
from pkgwh.util._dyn_libs.display_preserved_libs import \
	display_preserved_libs
from pkgwh.util._info_files import chk_updated_info_files
...
def post_emerge(myaction, myopts, myfiles,
	...
	config_protect = pkgwh.util.shlex_split(
	...
	vdb_path = os.path.join(root_config.settings['EROOT'], pkgwh.VDB_PATH)
	...
	postemerge = os.path.join(settings["PORTAGE_CONFIGROOT"],
		pkgwh.USER_CONFIG_PATH, "bin", "post_emerge")
	if os.access(postemerge, os.X_OK):
		hook_retval = pkgwh.process.spawn(
