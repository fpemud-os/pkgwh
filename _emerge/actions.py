...
import pkgwh
pkgwh.proxy.lazyimport.lazyimport(globals(),
	'pkgwh.dbapi._similar_name_search:similar_name_search',
	'pkgwh.debug',
	'pkgwh.news:count_unread_news,display_news_notifications',
	'pkgwh.util._get_vm_info:get_vm_info',
	'pkgwh.util.locale:check_locale',
	'pkgwh.emaint.modules.sync.sync:SyncRepos',
...
from pkgwh import _encodings, _unicode_decode
from pkgwh.binrepo.config import BinRepoConfigLoader
from pkgwh.const import BINREPOS_CONF_FILE, _DEPCLEAN_LIB_CHECK_DEFAULT
from pkgwh.dbapi.dep_expand import dep_expand
from pkgwh.dbapi._expand_new_virt import expand_new_virt
from pkgwh.dbapi.IndexedPortdb import IndexedPortdb
from pkgwh.dbapi.IndexedVardb import IndexedVardb
from pkgwh.dep import Atom, _repo_separator, _slot_separator
from pkgwh.exception import InvalidAtom, InvalidData, ParseError
from pkgwh.output import colorize, create_color_func, darkgreen, \
	red, xtermTitle, xtermTitleReset
good = create_color_func("GOOD")
bad = create_color_func("BAD")
warn = create_color_func("WARN")
from pkgwh.package.ebuild._ipc.QueryCommand import QueryCommand
from pkgwh.package.ebuild.fetch import _hide_url_passwd
from pkgwh._sets import load_default_config, SETPREFIX
from pkgwh._sets.base import InternalPackageSet
from pkgwh.util import cmp_sort_key, normalize_path, writemsg, varexpand, \
	writemsg_level, writemsg_stdout
from pkgwh.util.digraph import digraph
from pkgwh.util.SlotObject import SlotObject
from pkgwh.util._async.run_main_scheduler import run_main_scheduler
from pkgwh.util._async.SchedulerInterface import SchedulerInterface
from pkgwh.util._eventloop.global_event_loop import global_event_loop
from pkgwh._global_updates import _global_updates
from pkgwh.sync.old_tree_timestamp import old_tree_timestamp_warn
from pkgwh.localization import _
from pkgwh.metadata import action_metadata
from pkgwh.emaint.main import print_results
	...
	if '--getbinpkg' in emerge_config.opts or quickpkg_direct:
		...
		if quickpkg_direct:
			if quickpkg_root == emerge_config.running_config.settings['ROOT']:
				...
				quickpkg_settings = pkgwh.config(
				...
				quickpkg_vardb = pkgwh.vartree(settings=quickpkg_settings).dbapi
	...
	if resume:
		...
		try:
			...
			if isinstance(e, depgraph.UnsatisfiedResumeDep):
				...
				for line in textwrap.wrap(msg, 72):
					out.eerror(line)
			elif isinstance(e, pkgwh.exception.PackageNotFound):
		...
		if success:
			if dropped_tasks:
				pkgwh.writemsg("!!! One or more packages have been " + \
					"dropped due to\n" + \
		...
		except pkgwh.exception.PackageSetNotFound as e:
		...
		if mergecount == 0:
			...
			if retval == os.EX_OK and \
				...
				if "yes" == settings.get("AUTOCLEAN"):
					pkgwh.writemsg_stdout(">>> Auto-cleaning packages...\n")
					unmerge(trees[settings['EROOT']]['root_config'],
						myopts, "clean", [],
						ldpath_mtimes, autoclean=1)
				else:
					pkgwh.writemsg_stdout(colorize("WARN", "WARNING:")
...
def action_config(settings, trees, myopts, myfiles):
	...
	if not is_valid_package_atom(myfiles[0], allow_repo=True):
		pkgwh.writemsg("!!! '%s' is not a valid package atom.\n" % myfiles[0],
			noiselevel=-1)
		pkgwh.writemsg("!!! Please check ebuild(5) for full details.\n")
		pkgwh.writemsg("!!! (Did you specify a version but forget to prefix with '='?)\n")
		sys.exit(1)
	print()
	try:
		pkgs = trees[settings['EROOT']]['vartree'].dbapi.match(myfiles[0])
	except pkgwh.exception.AmbiguousPackageName as e:
...
def action_depclean(settings, trees, ldpath_mtimes,
	...
	if action == "depclean" and "--quiet" not in myopts and not myfiles:
		pkgwh.writemsg_stdout("\n")
		for x in msg:
			pkgwh.writemsg_stdout(colorize("WARN", " * ") + x)
	...
	for k in ("profile", "system", "selected"):
		...
		except pkgwh.exception.PackageSetNotFound:
...
def _calc_depclean(settings, trees, ldpath_mtimes,
			...
			for pkg in vardb:
				...
				try:
					if args_set.findAtomForPackage(pkg) is None:
						...
						continue
				except pkgwh.exception.InvalidDependString as e:
	...
	def show_parents(child_node):
		...
		for parent_str in parent_strs:
			msg.append("    %s\n" % (parent_str,))
		msg.append("\n")
		pkgwh.writemsg_stdout("".join(msg), noiselevel=-1)
		...
		if action == "depclean":
			if args_set:
				...
				for pkg in sorted(vardb, key=cmp_sort_key(cmp_pkg_cpv)):
					...
					except pkgwh.exception.InvalidDependString:
		...
		for atom in atoms:
			if not atom.startswith(SETPREFIX):
				if atom.cp.startswith("null/"):
					...
					null_cat, pn = pkgwh.catsplit(atom.cp)
					for world_atom in world_atoms:
						cat, world_pn = pkgwh.catsplit(world_atom.cp)
						if pn == world_pn:
							expanded_atoms.add(
...
class _info_pkgs_ver:
	...
	def __lt__(self, other):
		return pkgwh.versions.vercmp(self.ver, other.ver) < 0
...
def action_info(settings, trees, myopts, myfiles):
	...
	for repo in repos:
		last_sync = pkgwh.grabfile(os.path.join(repo.location, "metadata", "timestamp.chk"))
		head_commit = None
		if last_sync:
			append("Timestamp of repository %s: %s" % (repo.name, last_sync[0]))
		if repo.sync_type:
			sync = pkgwh.sync.module_controller.get_class(repo.sync_type)()
			options = { 'repo': repo }
			try:
	...
	basename = os.path.basename(os.path.realpath(os.path.join(
		os.sep, pkgwh.const.EPREFIX, "bin", "sh")))
	try:
		...
		try:
			...
			matches = (trees[trees._running_eroot]["vartree"].dbapi.
				match(basename))
		except pkgwh.exception.AmbiguousPackageName:
	...
	myvars += pkgwh.util.grabfile(settings["PORTDIR"]+"/profiles/info_pkgs")
	atoms = []
	for x in myvars:
		try:
			x = Atom(x)
	...
	for orig_atom, x in myvars:
			...
			for cpv in pkg_matches:
				matched_cp = pkgwh.versions.cpv_getkey(cpv)
				ver = pkgwh.versions.cpv_getversion(cpv)
	...
	if "--verbose" in myopts:
		...
		myvars.extend(pkgwh.util.grabfile(settings["PORTDIR"]+"/profiles/info_vars"))
	...
	if mypkgs:
		...
		pkgsettings = pkgwh.config(clone=settings)
		...
		for mypkg in mypkgs:
			...
			if pkg_type == "installed":
				...
				ebuild_file_contents = pkgwh.xpak.tbz2(tbz2_file).getfile(ebuild_file_name)
			...
			if pkg_type == "installed":
				pkgwh.doebuild(ebuildpath, "info", settings=pkgsettings,
	...
	for x in files:
		if is_valid_package_atom(x, allow_repo=True) or \
			...
			for cpv in vardb.cpv_all():
				if pkgwh.match_from_list(ext_atom, [cpv]):
					require_metadata = False
					atom = pkgwh.cpv_getkey(cpv)
					if ext_atom.operator == '=*':
						atom = "=" + atom + "-" + \
							pkgwh.versions.cpv_getversion(cpv)
					if ext_atom.slot:
					...
					if require_metadata:
						try:
							...
							continue
						if not pkgwh.match_from_list(atom, [cpv]):
							continue
	...
	if lookup_owners:
		...
		if owners:
			for cpv in owners:
				...
				valid_atoms.append(pkgwh.dep.Atom(atom))
		else:
...
def adjust_config(myopts, settings):
	...
	try:
		...
		pkgwh.writemsg("!!! %s\n" % str(e), noiselevel=-1)
		pkgwh.writemsg("!!! Unable to parse integer: CLEAN_DELAY='%s'\n" % \
	...
	try:
		...
		pkgwh.writemsg("!!! %s\n" % str(e), noiselevel=-1)
		pkgwh.writemsg("!!! Unable to parse integer: EMERGE_WARNING_DELAY='%s'\n" % \
	...
	try:
		...
		if PORTAGE_DEBUG not in (0, 1):
			pkgwh.writemsg("!!! Invalid value: PORTAGE_DEBUG='%i'\n" % \
				PORTAGE_DEBUG, noiselevel=-1)
			pkgwh.writemsg("!!! PORTAGE_DEBUG must be either 0 or 1\n",
				noiselevel=-1)
			PORTAGE_DEBUG = 0
	except ValueError as e:
		pkgwh.writemsg("!!! %s\n" % str(e), noiselevel=-1)
		pkgwh.writemsg("!!! Unable to parse integer: PORTAGE_DEBUG='%s'\n" %\
...
def getportageversion(portdir, _unused, profile, chost, vardb):
	...
	libcver = []
	libclist = set()
	for atom in expand_new_virt(vardb, pkgwh.const.LIBC_PACKAGE_ATOM):
		if not atom.blocker:
			libclist.update(vardb.match(atom))
	if libclist:
		for cpv in sorted(libclist):
			libc_split = pkgwh.catpkgsplit(cpv)[1:]
			if libc_split[-1] == "r0":
	...
	return "Portage %s (%s, %s, %s, %s, %s)" % \
		(pkgwh.VERSION, pythonver, profilever, gccver, ",".join(libcver), unameout)
...
def load_emerge_config(emerge_config=None, env=None, **kargs):
	...
	for k, envvar in (("config_root", "PORTAGE_CONFIGROOT"), ("target_root", "ROOT"),
		...
		if v is not None:
			kwargs[k] = v
	emerge_config.trees = pkgwh.create_trees(trees=emerge_config.trees,
				**kwargs)
	...
	emerge_config.target_config.mtimedb = pkgwh.MtimeDB(
		os.path.join(target_eroot, pkgwh.CACHE_PATH, "mtimedb"))
...
def getgccversion(chost=None):
	...
	pkgwh.writemsg(gcc_not_found_error, noiselevel=-1)
	return "[unavailable]"
...
def ionice(settings):
	...
	if ionice_cmd:
		ionice_cmd = pkgwh.util.shlex_split(ionice_cmd)
	if not ionice_cmd:
		return

	variables = {"PID" : str(pkgwh.getpid())}
	cmd = [varexpand(x, mydict=variables) for x in ionice_cmd]

	try:
		rval = pkgwh.process.spawn(cmd, env=os.environ)
	except pkgwh.exception.CommandNotFound:
...
def missing_sets_warning(root_config, missing_sets):
	...
	global_config_path = pkgwh.const.GLOBAL_CONFIG_PATH
	if pkgwh.const.EPREFIX:
		global_config_path = os.path.join(pkgwh.const.EPREFIX,
				pkgwh.const.GLOBAL_CONFIG_PATH.lstrip(os.sep))
	msg.append("        This usually means that '%s'" % \
		(os.path.join(global_config_path, "sets/pkgwh.conf"),))
...
def run_action(emerge_config):
	...
	for fmt in emerge_config.target_config.settings.get("PORTAGE_BINPKG_FORMAT", "").split():
		if not fmt in pkgwh.const.SUPPORTED_BINPKG_FORMATS:
	...
	if "--quiet" not in emerge_config.opts:
		pkgwh.deprecated_profile_check(
	...
	if pkgwh.data.secpass < 2:
		...
		if "--pretend" not in emerge_config.opts and \
			...
			need_superuser = emerge_config.action in ('clean', 'depclean',
				...
				(fetchonly or \
				(buildpkgonly and pkgwh.data.secpass >= 1) or \
				emerge_config.action in ("metadata", "regen", "sync"))
			if pkgwh.data.secpass < 1 or \
				need_superuser:
				if need_superuser:
				...
				if "--ask" in emerge_config.opts:
					...
					if pkgwh.data.secpass < 1 and not need_superuser:
						pkgwh.data.portage_group_warning()
	...
	if not disable_emergelog:
		...
		if emerge_log_dir:
			try:
				...
				pkgwh.util.ensure_dirs(emerge_log_dir)
			except pkgwh.exception.PortageException as e:
				writemsg_level("!!! Error creating directory for " + \
					"EMERGE_LOG_DIR='%s':\n!!! %s\n" % \
					(emerge_log_dir, e),
					noiselevel=-1, level=logging.ERROR)
				pkgwh.util.ensure_dirs(_emerge.emergelog._emerge_log_dir)
			else:
				_emerge.emergelog._emerge_log_dir = emerge_log_dir
		else:
			_emerge.emergelog._emerge_log_dir = os.path.join(os.sep,
				pkgwh.const.EPREFIX.lstrip(os.sep), "var", "log")
			pkgwh.util.ensure_dirs(_emerge.emergelog._emerge_log_dir)
	...
	def emergeexit():
		...
		if xterm_titles:
			xtermTitleReset()
	pkgwh.atexit_register(emergeexit)
