...
from pkgwh.checksum import _hash_filter
from pkgwh.elog.messages import eerror
from pkgwh.package.ebuild.fetch import (
	_check_distfile,
...
from pkgwh.util._pty import _create_pty_or_pipe
...
class _EbuildFetcherProcess(ForkProcess):
	...
	def _check_already_fetched(self, settings, uri_map):
		...
		out = io.StringIO()
		eout = pkgwh.output.EOutput()
		eout.quiet = settings.get("PORTAGE_QUIET") == "1"
		success = True
		try:
			sys.stdout = out
			sys.stderr = out
			if pkgwh.output.havecolor:
				pkgwh.output.havecolor = not self.background
			...
			for filename in uri_map:
				...
				if not ok:
					success = False
					break
		except pkgwh.exception.FileNotFound:
	...
	def _start(self):
		...
		pkgwh.doebuild_environment(ebuild_path, 'fetch',
	...
	def _run(self):
		...
		pkgwh.output.havecolor = self._settings.get('NOCOLOR') \
			not in ('yes', 'true')
	...
	def _prefetch_size_ok(self, uri_map, settings, ebuild_path):
		...
		sizes = {}
		for filename in uri_map:
			# Use stat rather than lstat since pkgwh.fetch() creates
			# symlinks when PORTAGE_RO_DISTDIRS is used.
			try:
				st = os.stat(os.path.join(distdir, filename))
			except OSError:
