...
class JobStatusDisplay:
	...
	def __init__(self, quiet=False, xterm_titles=True):
		...
		if self._isatty:
			width = pkgwh.output.get_term_size()[1]
		else:
			width = 80
	...
	def _init_term(self):
		...
		for k, capname in self._termcap_name_map.items():
			...
			code = tigetstr(pkgwh._native_string(capname))
			if code is None:
	...
	def _display_status(self):
		...
		style_file = pkgwh.output.ConsoleStyleFile(color_output)
		style_file.write_listener = plain_output
		style_writer = pkgwh.output.StyleWriter(file=style_file, maxcol=9999)
