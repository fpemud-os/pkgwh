...
from pkgwh.elog.messages import eerror
from pkgwh.output import colorize, create_color_func, red
bad = create_color_func("BAD")
from pkgwh._sets import SETPREFIX
from pkgwh._sets.base import InternalPackageSet
from pkgwh.util import ensure_dirs, writemsg, writemsg_level
...
from pkgwh.package.ebuild.digestcheck import digestcheck
from pkgwh.package.ebuild.digestgen import digestgen
from pkgwh.package.ebuild.doebuild import (_check_temp_dir,
...
class Scheduler(PollScheduler):
	...
	class _unknown_internal_error(pkgwh.exception.PortageException):
		"""
	...
	def __init__(self, settings, trees, mtimedb, myopts,
		...
		if "parallel-fetch" in features and \
			...
			if "distlocks" not in features:
				pkgwh.writemsg(red("!!!")+"\n", noiselevel=-1)
				pkgwh.writemsg(red("!!!")+" parallel-fetching " + \
					"requires the distlocks feature enabled"+"\n",
					noiselevel=-1)
				pkgwh.writemsg(red("!!!")+" you have it disabled, " + \
		...
		portage_match = self._running_root.trees["vartree"].dbapi.match(
			pkgwh.const.PORTAGE_PACKAGE_ATOM)
		if portage_match:
	...
	def _handle_self_update(self):
		...
		for x in self._mergelist:
			...
			if x.root != self._running_root.root:
				continue
			if not pkgwh.dep.match_from_list(
				pkgwh.const.PORTAGE_PACKAGE_ATOM, [x]):
				continue
	...
	class _pkg_failure(pkgwh.exception.PortageException):
		"""
		...
		def __init__(self, *pargs):
			pkgwh.exception.PortageException.__init__(self, pargs)
			if pargs:
				self.status = pargs[0]
	...
	def _schedule_setup(self, setup_phase):
		...
		if self._task_queues.merge.max_jobs > 1 and \
			...
			# since the pkgwh.locks module does not behave as desired
	...
	def _check_manifests(self):
		...
		for myroot, pkgsettings in self.pkgsettings.items():
			quiet_config = pkgwh.config(clone=pkgsettings)
	...
	def _run_pkg_pretend(self, loop=None):
		...
		for x in self._mergelist:
			...
			try:
				...
				pkgwh.package.ebuild.doebuild.doebuild_environment(ebuild_path,
				...
				pkgwh.elog.elog_process(x.cpv, settings)
			finally:
	...
	def merge(self):
		...
		for root in self.trees:
			...
			self.pkgsettings[root] = pkgwh.config(
		...
		if background and not failure_log_shown and \
			...
			for mysettings, key, logentries in self._failed_pkgs_die_msgs:
				...
				print()
				for phase in pkgwh.const.EBUILD_PHASES:
	...
	def _elog_listener(self, mysettings, key, logentries, fulltext):
		errors = pkgwh.elog.filter_loglevels(logentries, ["ERROR"])
		if errors:
	...
	def _merge(self):
		...
		failed_pkgs = self._failed_pkgs
		pkgwh.locks._quiet = self._background
		pkgwh.elog.add_listener(self._elog_listener)
		...
		try:
			...
			pkgwh.locks._quiet = False
			pkgwh.elog.remove_listener(self._elog_listener)
	...
	def _allocate_config(self, root):
		...
		if self._config_pool[root]:
			...
			temp_settings = pkgwh.config(clone=self.pkgsettings[root])
	...
	def _calc_resume_list(self):
		...
		for task, atoms in dropped_tasks.items():
			...
			# not valid here.
			settings.pop("T", None)
			pkgwh.elog.elog_process(pkg.cpv, settings)
