...
from pkgwh.localization import localized_size
from pkgwh.output import bold, darkgreen, green, red
from pkgwh.util import writemsg_stdout
from pkgwh.util.iterators.MultiIterGroupBy import MultiIterGroupBy
...
class search:
	...
	def _xmatch(self, level, atom):
		...
		if level == "match-all":
			...
			result = list(x for x in matches if pkgwh.cpv_getkey(x) == cp)
			...
			for db in self._dbs:
				if hasattr(db, "xmatch"):
					...
					if not cpv or pkgwh.cpv_getkey(cpv) != cp:
						continue
					if not result or cpv == pkgwh.best([cpv, result]):
						result = cpv
				else:
					db_keys = list(db._aux_cache_keys)
	...
	def getInstallationStatus(self,package):
		if not isinstance(package, pkgwh.dep.Atom):
			package = pkgwh.dep.Atom(package)
		...
		if installed_package:
			try:
				...
				installed_package = pkgwh.best(installed_package)
	...
	def getVersion(self,full_package,detail):
		if len(full_package) > 1:
			package_parts = pkgwh.catpkgsplit(full_package)
