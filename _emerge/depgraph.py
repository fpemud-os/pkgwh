...
from pkgwh import _unicode_decode, _unicode_encode, _encodings
from pkgwh.const import pkgwh_PACKAGE_ATOM, USER_CONFIG_PATH, VCS_DIRS
from pkgwh.dbapi import dbapi
from pkgwh.dbapi.dep_expand import dep_expand
from pkgwh.dbapi.DummyTree import DummyTree
from pkgwh.dbapi.IndexedPortdb import IndexedPortdb
from pkgwh.dbapi._similar_name_search import similar_name_search
from pkgwh.dep import Atom, best_match_to_list, extract_affecting_use, \
	check_required_use, human_readable_required_use, match_from_list, \
	_repo_separator
from pkgwh.dep._slot_operator import (ignore_built_slot_operator_deps,
	strip_slots)
from pkgwh.eapi import eapi_has_strong_blocks, eapi_has_required_use, \
	_get_eapi_attrs
from pkgwh.exception import (InvalidAtom, InvalidData, InvalidDependString,
	PackageNotFound, PortageException)
from pkgwh.localization import _
from pkgwh.output import colorize, create_color_func, \
	darkgreen, green
bad = create_color_func("BAD")
from pkgwh.package.ebuild.config import _get_feature_flags
from pkgwh.package.ebuild.getmaskingstatus import \
	_getmaskingstatus, _MaskReason
from pkgwh._sets import SETPREFIX
from pkgwh._sets.base import InternalPackageSet
from pkgwh.util import ConfigProtect, shlex_split, new_protect_filename
from pkgwh.util import cmp_sort_key, writemsg, writemsg_stdout
from pkgwh.util import ensure_dirs, normalize_path
from pkgwh.util import writemsg_level, write_atomic
from pkgwh.util.digraph import digraph
from pkgwh.util.futures import asyncio
from pkgwh.util._async.TaskScheduler import TaskScheduler
from pkgwh.versions import _pkg_str, catpkgsplit
...
def _wildcard_set(atoms):
	...
	for x in atoms:
		...
		except pkgwh.exception.InvalidAtom:
			x = Atom("*/" + x, allow_wildcard=True, allow_repo=False)
		pkgs.add(x)
	return pkgs
...
class _frozen_depgraph_config:
	...
	def __init__(self, settings, trees, myopts, params, spinner):
		...
		for myroot in trees:
			...
			self.pkgsettings[myroot] = pkgwh.config(
...
class _dynamic_depgraph_config:
	...
	**self.digraph**
	...
	``pkgwh.util.digraph``, a directed graph data structure. ``pkgwh.util.digraph`` is used for a variety of
...
class depgraph:
		...
		for myroot in self._frozen_config.trees:
			...
			if not fake_vartree.dbapi:
				...
				self._frozen_config.pkgsettings[myroot] = \
					pkgwh.config(clone=fake_vartree.settings)
			...
			for atom in atoms:
				...
				if not atom.package:
					...
					if replacement_parent.built and \
						pkgwh.dep._match_slot(atom, dep.child):
						# We are searching for a replacement_parent
						# atom that will pull in a different child,
						# so continue checking the rest of the atoms.
						continue
	...
	def _flatten_atoms(self, pkg, use):
		...
		for dep_key in pkg._dep_keys:
			...
			dep_string = pkgwh.dep.use_reduce(
	...
	def _changed_deps(self, pkg):
		...
		if ebuild is None:
			...
			try:
				built_deps = []
				for k in depvars:
					dep_struct = pkgwh.dep.use_reduce(
		...
		for dep_root, dep_string, dep_priority in deps:
				...
				try:
					if (with_test_deps and 'test' not in use_enabled and
						...
						test_deps = pkgwh.dep.use_reduce(dep_string,
					...
					dep_string = pkgwh.dep.use_reduce(dep_string,
						uselist=use_enabled,
						is_valid_flag=pkg.iuse.is_valid_flag,
						opconvert=True, token_class=Atom,
						eapi=pkg.eapi)
	...
	def _wrapped_add_pkg_dep_string(self, pkg, dep_root, dep_priority,
		...
		if debug:
			writemsg_level("\nParent:    %s\n" % (pkg,),
				noiselevel=-1, level=logging.DEBUG)
			dep_repr = pkgwh.dep.paren_enclose(dep_string,
	...
	def _dep_expand(self, root_config, atom_without_category):
		...
		"""
		null_cp = pkgwh.dep_getkey(insert_category_into_atom(
			atom_without_category, "null"))
		cat, atom_pn = pkgwh.catsplit(null_cp)
	...
	def _select_files(self, myfiles):
		...
		for x in myfiles:
			...
			if ext==".tbz2":
				...
				ebuild_path = pkgwh.util.normalize_path(os.path.abspath(x))
				pkgdir = os.path.dirname(ebuild_path)
				tree_root = os.path.dirname(os.path.dirname(pkgdir))
				cp = pkgdir[len(tree_root)+1:]
				error_msg = ("\n\n!!! '%s' is not in a valid ebuild repository "
					"hierarchy or does not exist\n") % x
				if not pkgwh.isvalidatom(cp):
					writemsg(error_msg, noiselevel=-1)
					return 0, myfavorites
				cat = pkgwh.catsplit(cp)[0]
				mykey = cat + "/" + os.path.basename(ebuild_path[:-7])
				if not pkgwh.isvalidatom("="+mykey):
					writemsg(error_msg, noiselevel=-1)
					return 0, myfavorites
				...
				if ebuild_path:
					...
					if mykey not in portdb.xmatch(
						"match-visible", pkgwh.cpv_getkey(mykey)):
				...
				if x.startswith(SETPREFIX):
					s = x[len(SETPREFIX):]
					if s not in sets:
						raise pkgwh.exception.PackageSetNotFound(s)
				...
				if expanded_atoms:
					...
					null_atom = Atom(insert_category_into_atom(x, "null"),
						allow_repo=True)
					cat, atom_pn = pkgwh.catsplit(null_atom.cp)
		...
		for arg in self._expand_set_args(args, add_to_digraph=True):
			for atom in sorted(arg.pset.getAtoms()):
				...
				try:
					...
					if pprovided and pkgwh.match_from_list(atom, pprovided):
	...
	def _greedy_slots(self, root_config, atom, blocker_lookahead=False):
		...
		while slots:
			slot = slots.pop()
			slot_atom = pkgwh.dep.Atom("%s:%s" % (highest_pkg.cp, slot))
	...
	def _select_atoms_highest_available(self, root, depstring,
		...
		if not isinstance(depstring, list):
			...
			depstring = pkgwh.dep.use_reduce(depstring,
		...
		if True:
			...
			try:
				...
				mycheck = pkgwh.dep_check(depstring, None,
			...
			if not mycheck[0]:
				raise pkgwh.exception.InvalidDependString(mycheck[1])
		if parent is None:
			...
			for cpv in cpv_list:
				for repo in repo_list:
					...
					if metadata is not None and \
						pkgwh.eapi_is_supported(metadata["EAPI"]):
						if not repo:
	...
	def _pkg_use_enabled(self, pkg, target_use=None):
		...
		def want_restart_for_use_change(pkg, new_use):
			...
			for key in Package._dep_keys + ("LICENSE",):
				...
				old_val = set(pkgwh.dep.use_reduce(dep, pkg.use.enabled, is_valid_flag=pkg.iuse.is_valid_flag, flat=True))
				new_val = set(pkgwh.dep.use_reduce(dep, new_use, is_valid_flag=pkg.iuse.is_valid_flag, flat=True))
					...
					if packages_with_invalid_use_config and (not built or not useoldpkg) and \
						...
						for rejected in packages_with_invalid_use_config:
							...
							if rejected > pkg:
								...
								break
							if pkgwh.dep.cpvequal(rejected.cpv, pkg.cpv):
		...
		if len(matched_packages) > 1:
			...
			if not visible_matches:
				visible_matches = [pkg.cpv for pkg in matched_packages \
					if self._pkg_visibility_check(pkg, autounmask_level)]
			if visible_matches:
				bestmatch = pkgwh.best(visible_matches)
			else:
				# all are masked, so ignore visibility
				bestmatch = pkgwh.best([pkg.cpv for pkg in matched_packages])
			matched_packages = [pkg for pkg in matched_packages \
				if pkgwh.dep.cpvequal(pkg.cpv, bestmatch)]
	...
	def _pkg(self, cpv, type_name, root_config, installed=False,
		...
		if pkg is None:
			...
			try:
				...
				raise pkgwh.exception.PackageNotFound(cpv)
			...
			for myroot in self._frozen_config.trees:
				...
				for pkg in vardb:
					...
					if blocker_data:
						...
						try:
							success, atoms = pkgwh.dep_check(depstr,
					...
					if blocker_atoms:
						...
						except pkgwh.exception.InvalidAtom as e:
							depstr = " ".join(vardb.aux_get(pkg.cpv, dep_keys))
							show_invalid_depstring_notice(
	...
	def _serialize_tasks(self):
		...
		if running_portage is not None:
			try:
				portage_rdepend = self._select_atoms_highest_available(
					running_root, running_pkgwh._metadata["RDEPEND"],
					myuse=self._pkg_use_enabled(running_portage),
					parent=running_portage, strict=False)
			except pkgwh.exception.InvalidDependString as e:
				pkgwh.writemsg("!!! Invalid RDEPEND in " + \
					"'%svar/db/pkg/%s/RDEPEND': %s\n" % \
					(running_root, running_pkgwh.cpv, e), noiselevel=-1)
				del e
		...
		for root in implicit_libc_roots:
			...
			if libc_pkgs:
				...
				for atom in self._expand_virt_from_graph(root,
					pkgwh.const.OS_HEADERS_PACKAGE_ATOM):
					if atom.blocker:
						continue
				...
				for task in myblocker_uninstalls.leaf_nodes():
					...
					if heuristic_overlap and running_root == task.root:
						...
						try:
							...
							pkgwh.writemsg("!!! Invalid PROVIDE in " + \
	...
	def _show_circular_deps(self, mygraph):
		...
		prefix = colorize("BAD", " * ")
		pkgwh.writemsg("\n", noiselevel=-1)
		pkgwh.writemsg(prefix + "Error: circular dependencies:\n",
			noiselevel=-1)
		pkgwh.writemsg("\n", noiselevel=-1)
		...
		if handler.circular_dep_message is not None:
			pkgwh.writemsg(handler.circular_dep_message, noiselevel=-1)
	...
	def _display_autounmask(self, autounmask_continue=False):
			...
			if pkg in self._dynamic_config.digraph:
				...
				for reason in mreasons:
					if reason.unmask_hint and \
						...
						comment, filename = pkgwh.getmaskingreason(
		...
		def find_config_file(abs_user_config, file_name):
			...
			pkgwh.util.grablines(recursive=True).
	...
	def saveNomergeFavorites(self):
		...
		for x in self._dynamic_config._set_nodes:
			...
			try:
				...
				writemsg("!!! see '%s'\n\n" % os.path.join(
					x.root, pkgwh.VDB_PATH, x.cpv, "PROVIDE"), noiselevel=-1)
				del e
		all_added = []
	...
	def _loadResumeCommand(self, resume_data, skip_masked=True,
		...
		for x in mergelist:
			...
			for atom in depgraph_sets.atoms.getAtoms():
				if atom.repo and pkgwh.dep.match_from_list(atom, [pkg_key]):
					repo = atom.repo
					break
			...
			if pkg is None:
				...
				if skip_missing:
					...
					continue
				raise pkgwh.exception.PackageNotFound(pkg_key)
	...
	class UnsatisfiedResumeDep(pkgwh.exception.PortageException):
		"""
		...
		def __init__(self, depgraph, value):
			pkgwh.exception.PortageException.__init__(self, value)
			self.depgraph = depgraph

	class _internal_exception(pkgwh.exception.PortageException):
...
def ambiguous_package_name(arg, atoms, root_config, spinner, myopts):
	...
	if "--quiet" in myopts:
		...
		for cp in sorted(set(pkgwh.dep_getkey(atom) for atom in atoms)):
...
def _spinner_start(spinner, myopts):
	...
	if show_spinner:
		pkgwh.writemsg_stdout("Calculating dependencies  ")
...
def get_mask_info(root_config, cpv, pkgsettings,
	...
	if metadata is None:
		...
		eapi = metadata['EAPI']
		if not pkgwh.eapi_is_supported(eapi):
...
def show_masked_packages(masked_packages):
	...
	for (root_config, pkgsettings, cpv, repo,
		...
		eapi_masked = metadata is not None and \
			not pkgwh.eapi_is_supported(metadata["EAPI"])
		if eapi_masked:
			have_eapi_mask = True
