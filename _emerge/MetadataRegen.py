...
class MetadataRegen(AsyncScheduler):
	...
	def _iter_metadata_processes(self):
		...
		pkgwh.writemsg_stdout("Regenerating cache entries...\n")
		for cp in self._cp_iter:
			if self._terminated.is_set():
				break
			cp_set.add(cp)
			pkgwh.writemsg_stdout("Processing %s\n" % cp)
		...
		if self._global_cleanse:
			for mytree in portdb.porttrees:
				try:
					...
					pkgwh.writemsg("Error listing cache entries for " + \
						"'%s': %s, continuing...\n" % (mytree, e),
						noiselevel=-1)
					del e
					dead_nodes = None
	...
	def _task_exit(self, metadata_process):
		...
		if metadata_process.returncode != os.EX_OK:
			...
			if not self._terminated_tasks:
				pkgwh.writemsg("Error processing %s, continuing...\n" % \
