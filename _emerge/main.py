...
from pkgwh.sync import _SUBMODULE_PATH_MAP
...
def emerge_main(args=None):
	...
	with open(os.devnull, 'r+b') as dev_null:
		...
		if pkgwh.process.spawn_bash("[[ $(< <(echo foo) ) == foo ]]",
			fd_pipes=fd_pipes) != 0:
			writemsg_level("Failed to validate a sane '/dev'.\n"
	...
	if "--ignore-default-opts" not in myopts:
		tmpcmdline.extend(pkgwh.util.shlex_split(
