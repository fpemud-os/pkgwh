...
from pkgwh.eapi import eapi_exports_replace_vars
from pkgwh.util import ensure_dirs
...
class Binpkg(CompositeTask):
	...
	def _start(self):
		...
		pkgwh.doebuild_environment(self._ebuild_path, 'setup',
		...
		if eapi_exports_replace_vars(settings["EAPI"]):
			...
			settings["REPLACING_VERSIONS"] = " ".join(
				set(pkgwh.versions.cpv_getversion(x) \
		...
		# pkgwh.locks functions are only designed
	...
	def _start_fetcher(self, lock_task=None):
		if lock_task is not None:
			...
			pkgwh.prepare_build_dirs(self.settings["ROOT"], self.settings, 1)
	...
	def _unpack_metadata(self, loop=None):
		...
		for mydir in (dir_path, self._image_dir, infloc):
			pkgwh.util.ensure_dirs(mydir, uid=pkgwh.data.portage_uid,
				gid=pkgwh.data.portage_gid, mode=dir_mode)
		...
		if pkg_path is not None:
			...
			if not md5sum:
				md5sum = pkgwh.checksum.perform_md5(pkg_path)
		...
		if env_extractor.returncode != os.EX_OK:
			raise pkgwh.exception.PortageException('failed to extract environment for {}'.format(self.pkg.cpv))
	...
	def _unpack_contents_exit(self, unpack_contents):
		...
		chpathtool = SpawnProcess(
			args=[pkgwh._python_interpreter,
	...
	def _async_unlock_builddir(self, returncode=None):
		...
		pkgwh.elog.elog_process(self.pkg.cpv, self.settings)
		self._start_task(
