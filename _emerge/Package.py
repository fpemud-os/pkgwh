...
from pkgwh.const import EBUILD_PHASES
from pkgwh.dep import Atom, check_required_use, use_reduce, \
	paren_enclose, _slot_separator, _repo_separator
from pkgwh.dep.soname.parse import parse_soname_deps
from pkgwh.versions import _pkg_str, _unknown_repo
from pkgwh.eapi import _get_eapi_attrs, eapi_has_use_aliases
from pkgwh.exception import InvalidData, InvalidDependString
...
class Package(Task):
	...
	def __init__(self, **kwargs):
		...
		self.category, self.pf = pkgwh.catsplit(self.cpv)
		self.cp = self.cpv.cp
	...
	def _eval_masks(self):
		...
		if pkgwh._eapi_is_deprecated(eapi):
	...
	def __str__(self):
		...
		s = "(%s, %s" \
			% (pkgwh.output.colorize(cpv_color, self.cpv +
	...
	def __lt__(self, other):
		...
		result = pkgwh.vercmp(self.version, other.version)
		if result < 0:
			return True
...
_all_metadata_keys = set(x for x in pkgwh.auxdbkeys \
