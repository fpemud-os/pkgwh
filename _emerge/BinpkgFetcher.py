...
class BinpkgFetcher(CompositeTask):
	...
	def _start(self):
		...
		if not self.pretend:
			pkgwh.util.ensure_dirs(os.path.dirname(self.pkg_path))
...
class _BinpkgFetcherProcess(SpawnProcess):
	...
	def _start(self):
		...
		fetch_args = [pkgwh.util.varexpand(x, mydict=fcmd_vars) \
			for x in pkgwh.util.shlex_split(fcmd)]
		...
		fd_pipes.setdefault(0, pkgwh._get_stdin().fileno())
	...
	class AlreadyLocked(pkgwh.exception.PortageException):
		pass
