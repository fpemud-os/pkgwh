...
from pkgwh.const import VDB_PATH
from pkgwh.dbapi.vartree import vartree
from pkgwh.dep._slot_operator import find_built_slot_operator_atoms
from pkgwh.eapi import _get_eapi_attrs
from pkgwh.exception import InvalidData, InvalidDependString
from pkgwh.update import grab_updates, parse_updates, update_dbentries
from pkgwh.versions import _pkg_str
...
class FakeVartree(vartree):
	...
	def _aux_get_wrapper(self, cpv, wants, myrepo=None):
		...
		try:
			live_metadata = dict(zip(self._portdb_keys,
				...
				myrepo=pkg.repo)))
		except (KeyError, pkgwh.exception.PortageException):
			live_metadata = None
	...
	def _apply_dynamic_deps(self, pkg, live_metadata):
		...
		try:
			...
			if not (pkgwh.eapi_is_supported(live_metadata["EAPI"]) and \
				pkgwh.eapi_is_supported(pkg.eapi)):
				raise _DynamicDepsNotApplicable()
...
def grab_global_updates(portdb):
	...
	for repo_name in portdb.getRepositories():
		...
		except pkgwh.exception.DirectoryNotFound:
			rawupdates = []
		upd_commands = []
