...
from pkgwh.util._dyn_libs.soname_deps_qa import (
	_get_all_provides,
	_get_unresolved_soname_deps,
)
from pkgwh.package.ebuild.prepare_build_dirs import (_prepare_workdir,
		_prepare_fake_distdir, _prepare_fake_filesdir)
from pkgwh.util.futures.compat_coroutine import coroutine
from pkgwh.util import writemsg
from pkgwh.util._async.AsyncTaskFuture import AsyncTaskFuture
from pkgwh.util._async.BuildLogger import BuildLogger
from pkgwh.util.futures import asyncio
from pkgwh.util.futures.executor.fork import ForkExecutor

try:
	from pkgwh.xml.metadata import MetaDataXML
...
pkgwh.proxy.lazyimport.lazyimport(globals(),
	'pkgwh.elog:messages@elog_messages',
	'pkgwh.package.ebuild.doebuild:_check_build_log,' + \
		...
		'_preinst_bsdflags',
	'pkgwh.util.futures.unix_events:_set_nonblocking',
)
...
class EbuildPhase(CompositeTask):
	...
	def _start_lock(self):
		if (self.phase in self._locked_phases and
			...
			lock_path = os.path.join(eroot, pkgwh.VDB_PATH + "-ebuild")
	...
	def _fail_clean(self):
		self.returncode = None
		pkgwh.elog.elog_process(self.settings.mycpv, self.settings)
		phase = "clean"
	...
	def _elog(self, elog_funcname, lines, background=None, loop=None):
		...
		global_havecolor = pkgwh.output.havecolor
		try:
			pkgwh.output.havecolor = \
				self.settings.get('NOCOLOR', 'false').lower() in ('no', 'false')
			for line in lines:
				elog_func(line, phase=phase, key=self.settings.mycpv, out=out)
		finally:
			pkgwh.output.havecolor = global_havecolor
		msg = out.getvalue()
		if msg:
			build_logger = None
			try:
