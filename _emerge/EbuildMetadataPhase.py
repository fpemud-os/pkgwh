...
pkgwh.proxy.lazyimport.lazyimport(globals(),
	'pkgwh.package.ebuild._metadata_invalid:eapi_invalid',
)
from pkgwh import os
from pkgwh import _encodings
from pkgwh import _unicode_decode
from pkgwh import _unicode_encode
from pkgwh.dep import extract_unpack_dependencies
from pkgwh.eapi import eapi_has_automatic_unpack_dependencies
...
class EbuildMetadataPhase(SubProcess):
	...
	def _start(self):
		...
		with io.open(_unicode_encode(ebuild_path,
			...
			errors='replace') as f:
			self._eapi, self._eapi_lineno = pkgwh._parse_eapi_ebuild_head(f)
		...
		self.eapi_supported = pkgwh.eapi_is_supported(parsed_eapi)
		...
		retval = pkgwh.doebuild(ebuild_path, "depend",
	...
	def _async_waitpid_cb(self, *args, **kwargs):
		...
		if self.returncode == os.EX_OK and \
			...
			metadata_valid = True
			if len(pkgwh.auxdbkeys) != len(metadata_lines):
				# Don't trust bash's returncode if the
				# number of lines is incorrect.
				metadata_valid = False
			else:
				metadata = dict(zip(pkgwh.auxdbkeys, metadata_lines))
				parsed_eapi = self._eapi
				if parsed_eapi is None:
					parsed_eapi = "0"
				self.eapi_supported = \
