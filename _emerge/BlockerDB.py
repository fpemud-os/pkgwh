	...
	def findInstalledBlockers(self, new_pkg):
			...
			if cached_blockers is not None:
				...
				if not success:
					pkg_location = os.path.join(inst_pkg.root,
						pkgwh.VDB_PATH, inst_pkg.category, inst_pkg.pf)
					pkgwh.writemsg("!!! %s/*DEPEND: %s\n" % \
		...
		if blocker_atoms:
			...
			for inst_pkg in installed_pkgs:
				...
				except (pkgwh.exception.InvalidDependString, StopIteration):
					continue
